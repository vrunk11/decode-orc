 ...
 * Purpose:     Stage API — stable types and interfaces for stage plugin implementation
 *
 ...
 *   - ParameterizedStage mixin (optional; stages that expose configurable parameters)
 *   - TriggerableStage mixin (optional; sink stages that can be triggered to run)
 ...
 *   - VideoSystem, SourceType (format/source enums used in parameter descriptors)
 *
...
// Node metadata types: NodeTypeInfo, NodeType, VideoFormatCompatibility, SinkCategory.
#include <node_type.h>

// ParameterizedStage mixin interface: get_parameter_descriptors(), get_parameters(),
// set_parameters(). Include this when your stage exposes configurable parameters.
#include <stage_parameter.h>

// TriggerableStage mixin interface: trigger(), get_trigger_status(), etc.
// Include-guarded; only defines the interface; no MVP enforcement on plugin builds.
