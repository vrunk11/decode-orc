...
bool RenderPresenter::applyStageParameters(
  ...
  // Live preview execution paths pass DAGNode::parameters back into
  // stage::execute(). Keep DAG node parameters in sync with the applied live
  // tweaks so execute() does not immediately overwrite stage state with stale
  // persisted values.
  ...
  for (auto& node : dag_nodes) {
    if (node.node_id == node_id) {
      ...
      if (!param_stage) {
        ORC_LOG_WARN(
            "RenderPresenter::applyStageParameters: node '{}' stage is not "
            "ParameterizedStage",
            node_id.to_string());
        return false;
      }
      ...
      ORC_LOG_DEBUG(
          "RenderPresenter::applyStageParameters: node '{}' set_parameters -> "
          "{}",
  ...
  ORC_LOG_WARN(
      "RenderPresenter::applyStageParameters: node '{}' not found in DAG",
      node_id.to_string());
  return false;
}

std::vector<orc::LiveTweakableParameterView>
RenderPresenter::getStageTweakableParameters(NodeID node_id) {
  auto dag = impl_->getConcreteDAG();
  if (!dag) {
    return {};
  }
  ...
  for (const auto& node : dag->nodes()) {
    if (node.node_id == node_id) {
      auto* cap_stage =
          dynamic_cast<const orc::IStagePreviewCapability*>(node.stage.get());
      if (!cap_stage) {
        return {};
      }
      ...
      for (const auto& tp : cap.tweakable_parameters) {
        ...
        view.tweak_class =
            (tp.tweak_class == orc::PreviewTweakClass::DisplayPhase)
...
RenderPresenter::getStageCurrentParameters(NodeID node_id) {
  auto dag = impl_->getConcreteDAG();
  if (!dag) {
    return {};
  }
  ...
  for (const auto& node : dag->nodes()) {
    ...
    auto* param_stage =
        dynamic_cast<const orc::ParameterizedStage*>(node.stage.get());
    if (!param_stage) {
      return {};
    }
