...
void FieldTimingDialog::setFieldData(
    const QString& node_id, uint64_t field_index,
    const std::vector<uint16_t>& samples, std::optional<uint64_t> field_index_2,
  ...
  if (field_index_2.has_value()) {
    title +=
        QString(" + %1").arg(field_index_2.value() + 1);  // Convert to 1-based
  }
