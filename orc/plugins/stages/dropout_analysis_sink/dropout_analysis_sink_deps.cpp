...
namespace orc
{
        ...
        DropoutAnalysisComputeOptions options)
    {
        ...
        logger_.debug(
            "DropoutAnalysisSinkDeps: {} total fields ({} total frames), binning by {} fields per data point",
            total_fields,
            total_frames,
            fields_per_bin);
        ...
        for (const auto& [frame_number, accum] : frame_accum) {
            ...
            if (frames_in_bin >= frames_per_bin) {
                logger_.debug(
                    "DropoutAnalysisSinkDeps: Bucket {} - frames {}-{}: total_dropout_length={:.2f}, dropout_count={:.2f} ({} frames)",
                    result.frame_stats.size(),
                    bin_start_frame,
                    frame_number,
                    current_bin.total_dropout_length,
                    current_bin.dropout_count,
                    frames_in_bin);
        ...
        if (frames_in_bin > 0) {
            logger_.debug(
                "DropoutAnalysisSinkDeps: Final bucket {} - frames {}-{}: total_dropout_length={:.2f}, dropout_count={:.2f} ({} frames)",
                result.frame_stats.size(),
                bin_start_frame,
                current_bin.frame_number,
                current_bin.total_dropout_length,
                current_bin.dropout_count,
                frames_in_bin);
            result.frame_stats.push_back(current_bin);
        ...
        const std::vector<FrameDropoutStats>& frame_stats)
    {
        ...
        if (!csv.is_open()) {
            logger_.error("DropoutAnalysisSinkDeps: Failed to open file for writing: {}", path);
            return false;
        }
        ...
        logger_.debug("DropoutAnalysisSinkDeps: Successfully wrote {} data rows to: {}", rows_written, path);
        return true;
    }
} // namespace orc
