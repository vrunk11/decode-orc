...
ORC_STAGE_PLUGIN_EXPORT bool orc_register_stage_plugin(
  ...
  if (node_type_info.display_name !=
          orc::plugins::snr_analysis_sink::kStageDisplayName ||
      node_type_info.menu_category !=
          orc::plugins::snr_analysis_sink::kStageMenuCategory ||
      node_type_info.type != orc::plugins::snr_analysis_sink::kStageNodeType ||
      node_type_info.min_inputs !=
          orc::plugins::snr_analysis_sink::kStageMinInputs ||
      node_type_info.max_inputs !=
          orc::plugins::snr_analysis_sink::kStageMaxInputs ||
      node_type_info.min_outputs !=
          orc::plugins::snr_analysis_sink::kStageMinOutputs ||
      node_type_info.max_outputs !=
          orc::plugins::snr_analysis_sink::kStageMaxOutputs ||
      node_type_info.compatible_formats !=
          orc::plugins::snr_analysis_sink::kStageCompatibleFormats ||
      node_type_info.sink_category !=
          orc::plugins::snr_analysis_sink::kStageSinkCategory) {
    if (error_message) {
      *error_message =
  ...
  if (!register_stage(context, orc::plugins::snr_analysis_sink::kStageName,
                      &create_stage)) {
    if (error_message) {
