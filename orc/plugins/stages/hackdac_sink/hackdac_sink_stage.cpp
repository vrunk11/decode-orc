...
NodeTypeInfo HackdacSinkStage::get_node_type_info() const {
  return NodeTypeInfo{NodeType::SINK,
                      "hackdac_sink",
                      "HackDAC Sink",
                      "Exports signed 16-bit field data without half-line "
                      "padding for HackDAC (.hdac) output.",
                      1,
                      1,
                      0,
                      0,
...
std::vector<ParameterDescriptor> HackdacSinkStage::get_parameter_descriptors(
  ...
  descriptors.push_back(ParameterDescriptor{
      "output_path", "HackDAC Output Path",
      "Destination .hdac file (signed 16-bit). A companion .txt report will be "
      "written next to it.",
      ParameterType::FILE_PATH,
      ParameterConstraints{
          std::nullopt, std::nullopt, std::string(""), {}, true, std::nullopt},
      ".hdac"});
...
bool HackdacSinkStage::set_parameters(
...
HackdacSinkStage::ParsedConfig HackdacSinkStage::parse_config(
  ...
  if (out_it == parameters.end() ||
    ...
    throw std::runtime_error(
        "output_path parameter is required and must be a string");
  }
  ...
  if (cfg.output_path.size() < ext.size() ||
      cfg.output_path.substr(cfg.output_path.size() - ext.size()) != ext) {
    cfg.output_path += ext;
  }
