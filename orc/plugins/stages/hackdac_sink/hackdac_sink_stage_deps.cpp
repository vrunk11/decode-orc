...
namespace orc
{
    void HackdacSinkStageDeps::init(TriggerProgressCallback progress_callback, std::atomic<bool>* cancel_requested)
    {
    ...
    bool HackdacSinkStageDeps::write_report(const std::string& report_path,
                                            ...
                                            const std::optional<SourceParameters>& video_params) const
    {
        ...
        if (!report.is_open()) {
            ORC_LOG_WARN("HackdacSinkDeps: Failed to write report file {}", report_path);
            return false;
        }
        ...
        const HackdacSinkExportOptions& options)
    {
        ...
        if (!writer.open(options.output_path)) {
            return {false, 0, "Error: Failed to open output file: " + options.output_path};
        }
        ...
        for (FieldID fid = field_range.start; fid < field_range.end; fid = fid + 1) {
            ...
            if (field_data.empty()) {
                ORC_LOG_WARN("HackdacSinkDeps: Field {} is empty, writing zeros", fid.value());
            ...
            if (progress_callback_ && processed_fields % 10 == 0) {
                progress_callback_(processed_fields,
                                   total_fields,
        ...
        writer.close();
        write_report(options.report_path, resolved_system, line_width, processed_fields, video_params);
