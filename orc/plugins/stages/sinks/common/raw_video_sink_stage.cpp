...
NodeTypeInfo RawVideoSinkStage::get_node_type_info() const {
  return NodeTypeInfo{NodeType::SINK,
                      "raw_video_sink",
                      "Raw Video Sink",
                      "Decodes composite video to raw RGB/YUV/Y4M files. Uses "
                      "the same chroma decoders as FFmpeg Video Sink but "
                      "outputs uncompressed raw data. Trigger to export.",
                      1,  // min_inputs
                      1,  // max_inputs
                      0,  // min_outputs
                      0,  // max_outputs
...
std::vector<ParameterDescriptor> RawVideoSinkStage::get_parameter_descriptors(
  ...
  for (const auto& param : params) {
    ...
    if (param.name == "encoder_preset" || param.name == "encoder_crf" ||
        param.name == "encoder_bitrate" || param.name == "embed_audio" ||
        param.name == "embed_closed_captions" ||
        param.name == "hardware_encoder" || param.name == "prores_profile" ||
...
std::map<std::string, ParameterValue> RawVideoSinkStage::get_parameters()
    const {
  // Get base parameters
...
bool RawVideoSinkStage::set_parameters(
  ...
  if (it != params.end()) {
    if (std::holds_alternative<std::string>(it->second)) {
      ...
      if (format != "rgb" && format != "yuv" && format != "y4m") {
        ORC_LOG_ERROR(
            "RawVideoSink: Invalid output format '{}' - must be rgb, yuv, or "
            "y4m",
            format);
        return false;
      }
