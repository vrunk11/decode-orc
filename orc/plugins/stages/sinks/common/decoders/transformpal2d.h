...
class TransformPal2D : public TransformPal {
 public:
  ...
  void filterFields(const std::vector<SourceField>& inputFields,
                    int32_t startIndex, int32_t endIndex,
                    std::vector<const double*>& outputFields) override;

 protected:
  void filterField(const SourceField& inputField, int32_t outputIndex);
  void forwardFFTTile(int32_t tileX, int32_t tileY, int32_t startY,
                      int32_t endY, const SourceField& inputField);
  void inverseFFTTile(int32_t tileX, int32_t tileY, int32_t startY,
                      int32_t endY, int32_t outputIndex);
  void applyFilter();
