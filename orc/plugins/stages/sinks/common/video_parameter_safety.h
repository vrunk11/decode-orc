...
inline VideoParameterSafetyResult sanitize_video_parameters(
  ...
  if (original_active_start != params.active_video_start ||
      original_active_end != params.active_video_end) {
    params.first_active_field_line =
        std::clamp(params.first_active_field_line, 0, params.field_height);
    params.last_active_field_line =
        std::clamp(params.last_active_field_line, 0, params.field_height);
  }
  ...
  const int32_t derived_last_field_line =
      (params.last_active_frame_line + 1) / 2;
  if (params.first_active_field_line < 0 ||
      params.first_active_field_line >= params.field_height) {
    add_warning(
        "first_active_field_line is out of range and will be derived from "
        "frame bounds");
    params.first_active_field_line = derived_first_field_line;
  } else {
    params.first_active_field_line =
        std::clamp(params.first_active_field_line, 0, params.field_height - 1);
  }
  if (params.last_active_field_line <= 0 ||
      params.last_active_field_line > params.field_height) {
    add_warning(
        "last_active_field_line is out of range and will be derived from frame "
        "bounds");
    params.last_active_field_line = derived_last_field_line;
  } else {
    params.last_active_field_line =
        std::clamp(params.last_active_field_line, 1, params.field_height);
  }
  if (params.first_active_field_line >= params.last_active_field_line) {
    add_warning(
        "active field line range is inverted and will be derived from frame "
        "bounds");
    params.first_active_field_line = derived_first_field_line;
    params.last_active_field_line =
        std::max(derived_first_field_line + 1, derived_last_field_line);
    params.last_active_field_line =
        std::clamp(params.last_active_field_line, 1, params.field_height);
  }
  ...
  if (params.black_16b_ire >= params.white_16b_ire) {
    add_warning(
        "black_16b_ire must be lower than white_16b_ire; decoder-safe defaults "
        "will be used");
    const int32_t black_level =
        (params.black_16b_ire >= 0) ? params.black_16b_ire : 0;
  ...
  if (requires_colour_reference) {
    const bool sample_rate_valid =
        std::isfinite(params.sample_rate) && params.sample_rate > 0.0;
    ...
    if (params.colour_burst_start < 0 ||
        params.colour_burst_start >= max_burst_end) {
    ...
    params.colour_burst_start = std::clamp(params.colour_burst_start, 0,
                                           std::max(0, max_burst_end - 1));
    params.colour_burst_end =
        std::clamp(params.colour_burst_end, 1, max_burst_end);

    if (params.colour_burst_start >= params.colour_burst_end) {
      add_warning(
          "colour burst range is inverted and will be reset to a decoder-safe "
          "window");
      params.colour_burst_end = max_burst_end;
      params.colour_burst_start =
          std::max(0, params.colour_burst_end - kDefaultBurstLength);
      if (params.colour_burst_start >= params.colour_burst_end) {
    ...
    params.colour_burst_start = std::clamp(params.colour_burst_start, 0,
                                           std::max(0, params.field_width - 1));
    params.colour_burst_end =
        std::clamp(params.colour_burst_end, 0, params.field_width);
    if (params.colour_burst_start >= params.colour_burst_end) {
