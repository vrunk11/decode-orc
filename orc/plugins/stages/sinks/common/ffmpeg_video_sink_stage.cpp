...
NodeTypeInfo FFmpegVideoSinkStage::get_node_type_info() const {
  return NodeTypeInfo{
      ...
      "ffmpeg_video_sink",
      "FFmpeg Video Sink",
      "Decodes composite video to MP4/MKV with optional audio and subtitles. "
      "Uses the same chroma decoders as Raw Video Sink but outputs compressed "
      "video files. Trigger to export.",
      1,  // min_inputs
      1,  // max_inputs
      0,  // min_outputs
      0,  // max_outputs
...
FFmpegVideoSinkStage::get_parameter_descriptors(VideoSystem project_format,
                                                SourceType source_type) const {
  // Get base parameters from ChromaSinkStage
  auto params =
      ChromaSinkStage::get_parameter_descriptors(project_format, source_type);
  ...
  for (const auto& param : params) {
    ...
    if (param.name == "output_path") {
      ...
      modified_param.description =
          "Path to output video file (MP4, MKV, MOV, or MXF format)";
      filtered_params.push_back(modified_param);
      continue;
    }
...
std::map<std::string, ParameterValue> FFmpegVideoSinkStage::get_parameters()
    const {
  // Return all base parameters (includes FFmpeg-specific ones)
  return ChromaSinkStage::get_parameters();
}

bool FFmpegVideoSinkStage::set_parameters(
  ...
  if (it != params.end()) {
    if (std::holds_alternative<std::string>(it->second)) {
      ...
      if (format == "rgb" || format == "yuv" || format == "y4m") {
        ORC_LOG_ERROR(
            "FFmpegVideoSink: Invalid output format '{}' - use mp4-h264 or "
            "mkv-ffv1",
            format);
        return false;
      }
      ...
      if (!is_supported) {
        ORC_LOG_ERROR(
            "FFmpegVideoSink: Output format '{}' not supported (FFmpeg not "
            "available)",
            format);
        return false;
      }
#else
      ORC_LOG_ERROR(
          "FFmpegVideoSink: FFmpeg support not compiled in, cannot use format "
          "'{}'",
          format);
      return false;
#endif
      ...
      if (vfr) {
        ...
        for (FrameID frame_id = frame_range.first; frame_id <= frame_range.last;
          ...
          if (cancel_requested_.load()) {
            ORC_LOG_WARN(
                "FFmpegVideoSink: Cancelled during closed caption collection");
            return false;
          }
        }
