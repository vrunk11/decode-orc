 ...
 * File:        luma_filter.h
 * Module:      orc-core
 * Purpose:     Simple FIR filter for luma/chroma separation in dropout
 * correction
 *
...
class LumaFirFilter {
 public:
  ...
  static LumaFirFilter create_pal_filter() {
    ...
    // Designed to pass luma (~5.5 MHz) and attenuate chroma (~4.43 MHz
    // subcarrier)
    std::vector<double> coeffs = {0.0118, 0.0618, 0.1618, 0.2618, 0.3218,
  ...
  static LumaFirFilter create_ntsc_filter() {
    ...
    // Designed to pass luma (~3.6 MHz) and attenuate chroma (~3.58 MHz
    // subcarrier)
    std::vector<double> coeffs = {0.0085, 0.0515, 0.1515, 0.2515, 0.3115,
  ...
  void apply(const uint16_t* input, uint16_t* output, size_t width) const {
    ...
    for (size_t i = 0; i < width; ++i) {
      ...
      for (int32_t j = 0; j < static_cast<int32_t>(coeffs_.size()); ++j) {
        ...
        if (idx < 0) idx = 0;
        if (idx >= static_cast<int32_t>(width))
          idx = static_cast<int32_t>(width - 1);
 ...
 private:
  explicit LumaFirFilter(const std::vector<double>& coeffs) : coeffs_(coeffs) {}
