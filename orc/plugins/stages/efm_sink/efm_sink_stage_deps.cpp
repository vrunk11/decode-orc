...
namespace orc
{
    void EFMSinkStageDeps::init(TriggerProgressCallback progress_callback, std::atomic<bool>* cancel_requested)
    {
        ...
        const EFMSinkOptions& options)
    {
        ...
        ORC_LOG_DEBUG("EFMSinkDeps: Counting EFM t-values across {} fields", total_fields);
        ...
        for (FieldID fid = start_fid; fid < end_fid; ++fid) {
            ...
            if (fields_done % 10 == 0 && progress_callback_) {
                progress_callback_(
                    tvalues_accumulated,
                    total_tvalues,
                    "Buffering EFM: field " + std::to_string(fields_done) + "/" + std::to_string(total_fields));
            }
        }
        ...
        processor.beginStream(options.output_path, static_cast<int64_t>(efm_buffer.size()));
        ...
        if (!ok) {
            return {false, "Error: EFMSink: EfmProcessor::finishStream() returned false"};
        }
