...
void Interleave::deinterleave(std::vector<uint8_t> &inputData, std::vector<uint8_t> &inputError, std::vector<uint8_t> &inputPadded)
{
    // Ensure input data is 24 bytes long
    if (inputData.size() != 24) {
        ORC_LOG_ERROR("Interleave::deinterleave - Input data must be 24 bytes long");
        std::exit(1);
    }
