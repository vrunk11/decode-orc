...
void Rspc::qParityEcc(std::vector<uint8_t> &inputData, std::vector<uint8_t> &errorData)
{
    ...
    if (successfulCorrections >= 52) {
        ...
        ORC_LOG_DEBUG("Rspc::qParityEcc(): Q-Parity correction failed! Got {} correct out of 52 possible codewords", successfulCorrections);
    }
}
...
void Rspc::pParityEcc(std::vector<uint8_t> &inputData, std::vector<uint8_t> &errorData)
{
    ...
    if (successfulCorrections >= 86) {
        ...
        ORC_LOG_DEBUG("Rspc::pParityEcc(): P-Parity correction failed! Got {} correct out of 86 possible codewords", successfulCorrections);
    }
}
