...
bool WriterSector::open(const std::string& filename) {
  if (filename == "-") {
    ...
    if (!m_file.is_open()) {
      ORC_LOG_CRITICAL(
          "WriterSector::open() - Could not open file {} for writing",
          filename);
      return false;
    }
    ORC_LOG_DEBUG("WriterSector::open() - Opened file {} for data writing",
                  filename);
  }
  return true;
}
...
bool WriterSector::isStdout() const { return m_usingStdout; }
