...
bool WriterWavMetadata::open(const std::string& filename,
  ...
  if (!m_file.is_open()) {
    ORC_LOG_CRITICAL(
        "WriterWavMetadata::open() - Could not open file {} for writing",
        filename);
    return false;
  }
  ORC_LOG_DEBUG("WriterWavMetadata::open() - Opened file {} for data writing",
                filename);
...
void WriterWavMetadata::write(const AudioSection& audioSection) {
  if (!m_file.is_open()) {
    ORC_LOG_CRITICAL(
        "WriterWavMetadata::write() - File is not open for writing");
    return;
  }
  ...
  if (std::find(m_trackNumbers.begin(), m_trackNumbers.end(),
                metadata.trackNumber()) == m_trackNumbers.end()) {
    // Check that the new track number is greater than the previous track
    // numbers
    if (!m_trackNumbers.empty() &&
        metadata.trackNumber() < m_trackNumbers.back()) {
      ORC_LOG_WARN(
          "WriterWavMetadata::write() - Track number decreased from {} to {} - "
          "ignoring",
          m_trackNumbers.back(), metadata.trackNumber());
    } else {
      ...
      ORC_LOG_DEBUG(
          "WriterWavMetadata::write() - New track {} detected with disc start "
          "time {} and track start time {}",
          metadata.trackNumber(), m_absoluteSectionTime.toString(),
          m_sectionTime.toString());
    }
  }
  ...
  for (int subSection = 0; subSection < 98; ++subSection) {
    ...
    for (int sampleOffset = 0; sampleOffset < 12; sampleOffset += 2) {
      ...
      if (hasError && !m_inErrorRange) {
        ...
        m_errorRangeStart = convertToAudacityTimestamp(
            relativeSectionTime.minutes(), relativeSectionTime.seconds(),
        ...
        if (sampleOffset == 0) {
          ...
          if (subSection > 0) {
            rangeEnd = convertToAudacityTimestamp(
                relativeSectionTime.minutes(), relativeSectionTime.seconds(),
        ...
        std::string outputString = m_errorRangeStart + "\t" + rangeEnd +
                                   "\tError: " + sampleTimeStamp + "\n";
      ...
      // Concealed
      bool hasConcealed =
          concealed.at(sampleOffset) || concealed.at(sampleOffset + 1);

      if (hasConcealed && !m_inConcealedRange) {
        // Start of new error range
        m_concealedRangeStart = convertToAudacityTimestamp(
            relativeSectionTime.minutes(), relativeSectionTime.seconds(),
            relativeSectionTime.frameNumber(), subSection, sampleOffset);
...
void WriterWavMetadata::flush() {
  ...
  // Note: For track 1 the track time metadata might be wrong.  On some discs
  // the first track includes unmarked lead-in. Basically, at absolute disc time
  // of 00:00:00 the track time might be positive (e.g 00:01:74 or 2 seconds)
  // and then it will count down to 00:00:00 - at which point the track starts
  // and time starts counting up again.
  //
  // This isn't handled by the metadata writer, so the first track might have an
  // incorrect track start time (but the absolute time will be correct).
  ...
  if (m_trackNumbers.size() > 1) {
    ...
    for (size_t i = 0; i < m_trackNumbers.size(); ++i) {
      std::string trackAbsStartTime = convertToAudacityTimestamp(
          m_trackAbsStartTimes[i].minutes(), m_trackAbsStartTimes[i].seconds(),
          m_trackAbsStartTimes[i].frameNumber(), 0, 0);

      std::string trackAbsEndTime = convertToAudacityTimestamp(
          m_trackAbsEndTimes[i].minutes(), m_trackAbsEndTimes[i].seconds(),
    ...
    ORC_LOG_DEBUG(
        "WriterWavMetadata::flush(): Only 1 track present - not writing track "
        "metadata");
  }
}
...
std::string WriterWavMetadata::convertToAudacityTimestamp(int32_t minutes,
                                                          int32_t seconds,
                                                          int32_t frames,
                                                          int32_t subsection,
                                                          int32_t sample) {
  ...
  constexpr double SAMPLES_PER_SUBSECTION =
      6.0;  // 6 stereo samples per subsection
