...
bool WriterWav::open(const std::string& filename) {
  ...
  if (!m_file.is_open()) {
    ORC_LOG_CRITICAL("WriterWav::open() - Could not open file {} for writing",
                     filename);
    return false;
  }
  ORC_LOG_DEBUG("WriterWav::open() - Opened file {} for data writing",
                filename);
...
void WriterWav::close() {
  ...
  ORC_LOG_DEBUG(
      "WriterWav::close(): Filling out the WAV header before closing the wav "
      "file");
