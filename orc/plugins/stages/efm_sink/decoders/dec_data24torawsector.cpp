...
void Data24ToRawSector::processStateMachine()
{
    while (!m_inputBuffer.empty()) {
        ...
        for (int i = 0; i < 98; i++) {
            ...
            m_sectorData.insert(m_sectorData.end(), frameData.begin(), frameData.end());

            // Error data
            const std::vector<uint8_t>& frameErrorData = d24Frame.errorData();
            m_sectorErrorData.insert(m_sectorErrorData.end(), frameErrorData.begin(), frameErrorData.end());

            // Padded data
            const std::vector<uint8_t>& framePaddedData = d24Frame.paddedData();
            m_sectorPaddedData.insert(m_sectorPaddedData.end(), framePaddedData.begin(), framePaddedData.end());
        }
...
Data24ToRawSector::State Data24ToRawSector::waitingForSync()
{
    ...
    if (m_sectorData.size() < 2352) {
        // Not enough data
        ORC_LOG_DEBUG("Data24ToRawSector::waitingForSync(): Not enough data in sectorData to form a sector, waiting for more data");
    ...
    int syncPatternPosition = (it != m_sectorData.end()) ? std::distance(m_sectorData.begin(), it) : -1;
    ...
    if (syncPatternPosition == -1) {
        ...
        if (m_sectorData.size() > 11) {
            ...
            m_sectorErrorData.erase(m_sectorErrorData.begin(), m_sectorErrorData.end() - 11);
            m_sectorPaddedData.erase(m_sectorPaddedData.begin(), m_sectorPaddedData.end() - 11);
        }
        ...
        m_sectorData.erase(m_sectorData.begin(), m_sectorData.begin() + syncPatternPosition);
        m_sectorErrorData.erase(m_sectorErrorData.begin(), m_sectorErrorData.begin() + syncPatternPosition);
        m_sectorPaddedData.erase(m_sectorPaddedData.begin(), m_sectorPaddedData.begin() + syncPatternPosition);
        
        ORC_LOG_DEBUG("Data24ToRawSector::waitingForSync(): Possible sync pattern found in sectorData at position: {} discarding {} bytes", 
        ...
        // High padding count alone (from CIRC delay contamination after gap-filling sections)
        // does not indicate a false positive – only high error count does.
        if (errorByteCount > 1000 && paddingByteCount > 1000) {
            ORC_LOG_INFO("Data24ToRawSector::waitingForSync(): Discarding sync as false positive due to {} error bytes and {} padding bytes", 
                                      errorByteCount, paddingByteCount);
            nextState = WaitingForSync;
        } else {
            ORC_LOG_INFO("Data24ToRawSector::waitingForSync(): Accepting sync with {} error bytes and {} padding bytes",
                                      errorByteCount, paddingByteCount);
            nextState = InSync;
        }
    }
...
Data24ToRawSector::State Data24ToRawSector::inSync()
{
    ...
    if (m_sectorData.size() < 2352) {
        // Not enough data
        ORC_LOG_DEBUG("Data24ToRawSector::inSync(): Not enough data in sectorData to form a sector, waiting for more data");
        ...
        auto errorIt = std::find(m_sectorErrorData.begin(), m_sectorErrorData.begin() + 12, 1);
        auto paddingIt = std::find(m_sectorPaddedData.begin(), m_sectorPaddedData.begin() + 12, 1);
        
        if (errorIt != m_sectorErrorData.begin() + 12 || paddingIt != m_sectorPaddedData.begin() + 12) {
            // Is the sector broken?  Count the total number of error bytes and padding bytes in the sector
            int32_t errorByteCount = 0;
            int32_t paddingByteCount = 0;
            for (int i = 0; i < 2352; i++) {
            ...
            ORC_LOG_DEBUG("Data24ToRawSector::inSync(): Sector header corrupt. Sector contains {} error bytes and {} padding bytes",
                         errorByteCount, paddingByteCount);
        }

        // Is there a valid sync pattern at the beginning of the sector data?
        bool syncMatch = (m_sectorData.size() >= 12 && 
                         std::equal(m_syncPattern.begin(), m_syncPattern.end(), m_sectorData.begin()));
        ...
        if (!syncMatch) {
            ...
            if (m_missedSyncPatternCount > 4) {
                ...
                ORC_LOG_DEBUG("Data24ToRawSector::inSync(): Too many missed sync patterns (4 missed), lost sync. Valid sector count: {}", m_validSectorCount);
                nextState = LostSync;
                return nextState;
            } else {
                ...
                ORC_LOG_DEBUG("Data24ToRawSector::inSync(): Sync pattern mismatch - Found: {} Sector count: {} Missed sync patterns: {}",
            ...
            if (m_missedSyncPatternCount) {
                ORC_LOG_DEBUG("Data24ToRawSector::inSync(): Sync pattern found after {} missed sync patterns (resynced)", m_missedSyncPatternCount);
            }
        ...
        std::vector<uint8_t> rawDataOut(m_sectorData.begin(), m_sectorData.begin() + 2352);
        std::vector<uint8_t> rawErrorDataOut(m_sectorErrorData.begin(), m_sectorErrorData.begin() + 2352);
        std::vector<uint8_t> rawPaddedDataOut(m_sectorPaddedData.begin(), m_sectorPaddedData.begin() + 2352);
        ...
        m_sectorErrorData.erase(m_sectorErrorData.begin(), m_sectorErrorData.begin() + 2352);
        m_sectorPaddedData.erase(m_sectorPaddedData.begin(), m_sectorPaddedData.begin() + 2352);
    }
