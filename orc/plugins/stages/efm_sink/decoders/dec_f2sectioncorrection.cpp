...
void F2SectionCorrection::processQueue()
{
    ...
    if (m_noTimecodes && !m_leadinComplete) {
        ORC_LOG_DEBUG("F2SectionCorrection::processQueue(): No timecodes flag set, skipping leadin checks.");
    ...
    if (f2Section.metadata.isValid()) {
        ...
        if (!m_leadinBuffer.empty()) {
            ...
            if (f2Section.metadata.absoluteSectionTime() == expectedAbsoluteTime) {
                ...
                ORC_LOG_DEBUG("F2SectionCorrection::waitForInputToSettle(): Added valid section to leadin buffer with absolute time {}",
                ...
                if (m_leadinBuffer.size() >= 5) {
                    ...
                    ORC_LOG_DEBUG("F2SectionCorrection::waitForInputToSettle(): Leadin buffer complete, pushing collected sections for processing.");
                ...
                ORC_LOG_DEBUG("F2SectionCorrection::waitForInputToSettle(): Got section with invalid absolute time whilst waiting for input to settle (lead in buffer discarded).");
            }
        } else {
            // The leadin buffer is empty, so we can now add the collected section to the leadin buffer
            m_leadinBuffer.push_back(f2Section);
            ORC_LOG_DEBUG("F2SectionCorrection::waitForInputToSettle(): Added section to leadin buffer with valid metadata:");
            ORC_LOG_DEBUG("F2SectionCorrection::waitForInputToSettle():   Absolute time: {}", f2Section.metadata.absoluteSectionTime().toString());
            ORC_LOG_DEBUG("F2SectionCorrection::waitForInputToSettle():   Section time: {}", f2Section.metadata.sectionTime().toString());
            {
                ...
                ORC_LOG_DEBUG("F2SectionCorrection::waitForInputToSettle():   Section type: {}", sectionTypeStr);
            }
            ORC_LOG_DEBUG("F2SectionCorrection::waitForInputToSettle():   Track number: {}", f2Section.metadata.trackNumber());
        ...
        m_preLeadinSections += static_cast<int32_t>(m_leadinBuffer.size()) + 1;
        m_leadinBuffer.clear();
        ORC_LOG_DEBUG("F2SectionCorrection::waitForInputToSettle(): Got invalid metadata section whilst waiting for input to settle (lead in buffer discarded).");
    }
}
...
void F2SectionCorrection::waitingForSection(F2Section &f2Section)
{
    ...
    if (m_internalBuffer.empty()) {
        if (f2Section.metadata.isValid()) {
            ...
            ORC_LOG_DEBUG("F2SectionCorrection::waitingForSection(): Added section to internal buffer with absolute time {}",
                     f2Section.metadata.absoluteSectionTime().toString());
            return;
        } else {
            ORC_LOG_DEBUG("F2SectionCorrection::waitingForSection(): Got invalid metadata section whilst waiting for first section.");
            return;
        }
    }
    ...
    if (m_noTimecodes) {
        ...
        ORC_LOG_DEBUG("F2SectionCorrection::waitingForSection(): No timecodes flag set, setting section absolute time to expected time {}",
    ...
    if (f2Section.metadata.isValid() && (
        ...
        if (f2Section.metadata.qMode() == SectionMetadata::QMode2) {
            ORC_LOG_DEBUG("F2SectionCorrection::waitingForSection(): Q Mode 2 section detected, correcting absolute time to {}", correctedAbsoluteTime.toString());
        }

        if (f2Section.metadata.qMode() == SectionMetadata::QMode3) {
            ORC_LOG_DEBUG("F2SectionCorrection::waitingForSection(): Q Mode 3 section detected, correcting absolute time to {}", correctedAbsoluteTime.toString());
        }
    }
    ...
    if (f2Section.metadata.isValid()
        ...
        if (f2Section.metadata.absoluteSectionTime() > expectedAbsoluteTime) {
            ...
            if (missingSections > m_paddingWatermark) {
                ...
                ORC_LOG_WARN("F2SectionCorrection::waitingForSection(): Missing section gap of {} is larger than 5, expected absolute time is {} actual absolute time is {}",
                        missingSections, expectedAbsoluteTime.toString(), f2Section.metadata.absoluteSectionTime().toString());
                ORC_LOG_WARN("F2SectionCorrection::waitingForSection(): Gaps greater than {} frames will be treated as padding sections (i.e. the decoder thinks there is a gap in the EFM data rather than actual data loss).",
                        m_paddingWatermark);
            }

            if (missingSections == 1)
                ORC_LOG_WARN("F2SectionCorrection::waitingForSection(): Missing section detected, expected absolute time is {} actual absolute time is {}",
                        expectedAbsoluteTime.toString(), f2Section.metadata.absoluteSectionTime().toString());

            if (missingSections > 1)
                ORC_LOG_WARN("F2SectionCorrection::waitingForSection(): {} missing sections detected, expected absolute time is {} actual absolute time is {}",
                        missingSections, expectedAbsoluteTime.toString(), f2Section.metadata.absoluteSectionTime().toString());
            ...
            for (int i = 0; i < missingSections; ++i) {
                ...
                if (newFrames >= 0) {
                    ...
                    ORC_LOG_DEBUG("F2SectionCorrection::waitingForSection(): Negative section time detected, setting section time to 00:00:00");
                }
                ...
                if (missingSections <= m_paddingWatermark) {
                    ...
                    m_missingSections++;
                        ORC_LOG_DEBUG("F2SectionCorrection::waitingForSection(): Inserting missing section into internal buffer with absolute time: {} - marking all data as errors",
                            missingSection.metadata.absoluteSectionTime().toString());
                    for (int i = 0; i < 98; ++i) {
                        F2Frame errorFrame;
                        errorFrame.setData(std::vector<uint8_t>(32, 0x00));
                        errorFrame.setErrorData(std::vector<uint8_t>(32, 1)); // Flag as error
                        errorFrame.setPaddedData(std::vector<uint8_t>(32, 0)); // Not padded
                        missingSection.pushFrame(errorFrame);
                    }
                } else {
                    // Section is considered as padding, so fill it with valid data
                    m_paddingSections++;
                        ORC_LOG_DEBUG("F2SectionCorrection::waitingForSection(): Inserting missing section into internal buffer with absolute time: {} - marking all data as padding",
                            missingSection.metadata.absoluteSectionTime().toString());
                    for (int i = 0; i < 98; ++i) {
                        F2Frame errorFrame;
                ...
                ORC_LOG_DEBUG("F2SectionCorrection::waitingForSection(): Inserted missing section into internal buffer with absolute time {}",
                         missingSection.metadata.absoluteSectionTime().toString());
            }
        } else {
            // The current section is behind the expected section in time, so we have a section
            // that is out of order
            ORC_LOG_WARN("F2SectionCorrection::waitingForSection(): Section out of order detected, expected absolute time is {} actual absolute time is {}",
                expectedAbsoluteTime.toString(), f2Section.metadata.absoluteSectionTime().toString());
...
void F2SectionCorrection::processInternalBuffer()
{
    ...
    if (!m_internalBuffer.empty() && !m_internalBuffer.front().metadata.isValid()) {
        ORC_LOG_DEBUG("F2SectionCorrection::correctInternalBuffer(): Invalid section at start of internal buffer!");
    ...
    if (m_internalBuffer.size() < 3) {
        ORC_LOG_DEBUG("F2SectionCorrection::correctInternalBuffer(): Not enough sections in internal buffer to correct.");
        return;
    }
    ...
    for (int index = 1; index < static_cast<int>(m_internalBuffer.size()); ++index) {
        ...
        if (!m_internalBuffer[index].metadata.isValid()) {
            ...
            int timeDifference =
                ...
                ORC_LOG_DEBUG("F2SectionCorrection::correctInternalBuffer(): Section metadata invalid - Error between {} - {} gap length is {} time difference is {}",
            ...
            if (gapLength == timeDifference) {
                ...
                for (int i = errorStart + 1; i < errorEnd; ++i) {
                    ...
                    if (m_internalBuffer[errorStart].metadata.trackNumber()
                        ...
                        ORC_LOG_DEBUG("F2SectionCorrection::correctInternalBuffer(): Gap starts on track {} and ends on track {}",
                        ...
                        ORC_LOG_CRITICAL("F2SectionCorrection::correctInternalBuffer(): Exiting due to track change in internal buffer - untested functionality - please confirm!");
                    ...
                    m_correctedSections++;
                        ORC_LOG_DEBUG("F2SectionCorrection::correctInternalBuffer(): Corrected section {} with absolute time {}, Track number {} and track time {} from original metadata with absolute time {}",
...
void F2SectionCorrection::outputSections()
{
    ...
    if (m_trackNumbers.end() == std::find(m_trackNumbers.begin(), m_trackNumbers.end(), trackNumber)) {
        ...
        ORC_LOG_DEBUG("F2SectionCorrection::outputSections(): New track {} detected with start time {}", trackNumber,
            sectionTime.toString());

        if (trackNumber == 0 || trackNumber == 0xAA) {
            if (section.metadata.sectionType().type() == SectionType::LeadIn) {
                // This is a lead-in track
                ORC_LOG_DEBUG("F2SectionCorrection::outputSections(): LeadIn track detected with start time {}",
                    sectionTime.toString());
            } else if (section.metadata.sectionType().type() == SectionType::LeadOut) {
                // This is a lead-out track
                ORC_LOG_DEBUG("F2SectionCorrection::outputSections(): LeadOut track detected with start time {}",
                    sectionTime.toString());
            } else if (section.metadata.sectionType().type() == SectionType::UserData) {
                // This is a user data track
                ORC_LOG_DEBUG("F2SectionCorrection::outputSections(): UserData track detected with start time {}",
                    sectionTime.toString());
            } else {
                // This is an unknown track
                ORC_LOG_DEBUG("F2SectionCorrection::outputSections(): UNKNOWN track type detected with start time {}",
                    sectionTime.toString());
            }
        }
    } else {
...
void F2SectionCorrection::showStatistics() const
{
    ...
    if (m_totalSections > 0 && m_absoluteEndTime >= m_absoluteStartTime) {
        ORC_LOG_INFO("    Duration: {}", (m_absoluteEndTime - m_absoluteStartTime).toString());
    } else {
