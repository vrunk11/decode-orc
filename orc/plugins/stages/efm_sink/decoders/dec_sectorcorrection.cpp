  ...
  while (!m_inputBuffer.empty()) {
    ...
    if (!m_haveLastSectorInfo) {
      ...
      if (sector.address().frameNumber() != 0) {
        ...
        // sector address
        ORC_LOG_DEBUG(
            "SectorCorrection::processQueue(): First received frame address is "
            "{} ({})",
            sector.address().address(), sector.address().toString());
        ORC_LOG_DEBUG(
            "SectorCorrection::processQueue(): Filling missing leading sectors "
            "with {} sectors",
      ...
      if (sector.address() != m_lastSectorAddress + 1) {
        ...
        ORC_LOG_DEBUG(
            "SectorCorrection::processQueue(): Sector is not in the correct "
            "position. Last good sector address: {} {} Current sector address: "
            "{} {} Gap: {}",
