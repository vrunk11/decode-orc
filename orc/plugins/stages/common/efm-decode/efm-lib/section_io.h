...
static constexpr size_t kMetadataSize =
    33;  // 3×int32 + 7×uint8 + 2×uint32 + int32 + 2×uint8 = 33 bytes
...
inline bool writeUint32LE(std::ostream& out, uint32_t value) {
  uint8_t bytes[4] = {static_cast<uint8_t>((value >> 0) & 0xFF),
...
inline bool writeMetadata(std::ostream& out, const SectionMetadata& metadata) {
  if (!writeInt32LE(out, static_cast<int32_t>(metadata.sectionType().type())))
    return false;
...
inline bool readMetadata(std::istream& in, SectionMetadata& metadata,
                         uint32_t version) {
  ...
  metadata.setSectionType(
      SectionType(static_cast<SectionType::Type>(sectionTypeRaw)), trackNumber);
...
inline bool writeFrame(std::ostream& out, const FrameType& frame) {
  ...
  if (data.size() != frameSize || errorData.size() != frameSize ||
...
inline bool readF2Section(std::istream& in, F2Section& section,
                          uint32_t version) {
  section.clear();
...
inline bool writeData24Section(std::ostream& out,
                               const Data24Section& section) {
  if (!writeMetadata(out, section.metadata)) {
    return false;
  }
  for (size_t i = 0; i < kFramesPerSection; ++i) {
...
inline bool readData24Section(std::istream& in, Data24Section& section,
                              uint32_t version) {
  section.clear();
  SectionMetadata metadata;
  if (!readMetadata(in, metadata, version)) {
