...
class FieldMappedRepresentation : public VideoFieldRepresentationWrapper {
 public:
  FieldMappedRepresentation(
      ...
      std::vector<FieldID> field_mapping, const std::string& range_spec)
      : VideoFieldRepresentationWrapper(
            source,
            ArtifactID(
                "field_map_" + source->id().to_string() + "_" + range_spec +
                "_" +
    ...
    if (source_) {
      ...
      if (line_width == 0) {
        ...
        if (params && params->frame_width_nominal > 0) {
          line_width = static_cast<size_t>(params->frame_width_nominal);
        }
      }
  ...
  // Explicitly delete copy/move to prevent issues (this object is always used
  // via shared_ptr)
  FieldMappedRepresentation(const FieldMappedRepresentation&) = delete;
  FieldMappedRepresentation& operator=(const FieldMappedRepresentation&) =
      delete;
  ...
  size_t field_count() const override { return field_mapping_.size(); }
  ...
  std::optional<FieldDescriptor> get_descriptor(FieldID id) const override {
    ...
    if (!source_id.is_valid() && source_) {
      ...
      if (params) {
        ...
        desc.field_id = id;
        desc.width = static_cast<size_t>(params->frame_width_nominal);
        desc.height = calculate_padded_field_height(params->system);
 ...
 private:
  std::vector<FieldID>
      field_mapping_;  // Maps output field index -> source FieldID
  mutable std::vector<sample_type>
      black_line_;  // Cached black line for padding
};
...
std::vector<ArtifactPtr> FieldMapStage::execute(
  ...
  if (!source) {
    throw DAGExecutionError(
        "FieldMapStage input must be a VideoFieldRepresentation");
  }
  ...
  if (it != parameters.end()) {
    if (auto* str_val = std::get_if<std::string>(&it->second)) {
      if (*str_val != range_spec_) {
        ...
        if (ranges.empty()) {
          ORC_LOG_ERROR(
              "FieldMapStage: Failed to parse range specification: {}",
              range_spec);
  ...
  if (range_spec.empty() || ranges.empty()) {
    ORC_LOG_WARN(
        "FieldMapStage: No range specification provided, passing through "
        "unchanged");
  ...
  if (field_mapping.empty()) {
    ORC_LOG_WARN(
        "FieldMapStage: Range specification resulted in empty mapping");
    cached_output_ = source;  // Cache the input for preview rendering
    return {inputs[0]};
  }
  ...
  ORC_LOG_DEBUG(
      "FieldMapStage: Input has {} fields (range {}-{}), output will have {} "
      "fields based on specification: {}",
      source->field_count(), source_range.start.value(),
      source_range.end.value(), field_mapping.size(), range_spec);

  // Create wrapped representation with remapped fields
  auto result = std::make_shared<FieldMappedRepresentation>(
      source, std::move(field_mapping), range_spec);
  cached_output_ = result;
  return {result};
}

std::vector<ParameterDescriptor> FieldMapStage::get_parameter_descriptors(
  ...
  return {
      ...
      ParameterDescriptor{
          "seed", "Random Seed",
          "Random seed used to generate field corruption pattern (for "
          "reproducibility)",
          ParameterType::INT32,
          ParameterConstraints{
              std::nullopt,                // no min
              std::nullopt,                // no max
              ParameterValue{static_cast<int32_t>(0)},  // default: 0 (not set)
              {},                          // no allowed strings
              false,                       // not required
              std::nullopt                 // no dependency
          }}};
...
bool FieldMapStage::set_parameters(
      ...
      if (auto* str_val = std::get_if<std::string>(&value)) {
        ...
        if (!range_spec_.empty()) {
          ...
          if (cached_ranges_.empty()) {
            ORC_LOG_ERROR("FieldMapStage: Invalid range specification: {}",
                          range_spec_);
            return false;  // Invalid range specification
          }
          ORC_LOG_DEBUG(
              "FieldMapStage: Cached {} range(s) from specification: {}",
...
std::vector<std::pair<uint64_t, uint64_t>> FieldMapStage::parse_ranges(
  ...
  while (std::getline(iss, range_str, ',')) {
    ...
    if (range_str.substr(0, 4) == "PAD_") {
      try {
        ...
        ORC_LOG_DEBUG("FieldMapStage: Parsed padding directive: {} frames",
                      pad_count);
        continue;
      } catch (...) {
        ORC_LOG_ERROR("FieldMapStage: Invalid padding directive: {}",
                      range_str);
        return {};
      }
    }
    ...
    if (dash_pos == std::string::npos) {
      ...
      try {
        ...
        if (start > end) {
          ORC_LOG_ERROR("FieldMapStage: Invalid range (start > end): {}-{}",
                        start, end);
          return {};
        }
...
std::vector<FieldID> FieldMapStage::build_field_mapping(
  ...
  for (const auto& [start, end] : ranges) {
    ...
    for (uint64_t field_id = start; field_id <= end; ++field_id) {
      // Use field_id directly as absolute field ID, not as offset from
      // source_start
      ...
      if (field_id >= source_end) {
        ORC_LOG_WARN(
            "FieldMapStage: Field {} out of source range (0-{}), skipping",
      ...
      if (source.has_field(fid)) {
        ...
        ORC_LOG_WARN("FieldMapStage: Field {} not available in source",
                     field_id);
      }
    }
  }
...
PreviewImage FieldMapStage::render_preview(const std::string& option_id,
                                           uint64_t index,
