...
ORC_STAGE_PLUGIN_EXPORT bool orc_register_stage_plugin(
  ...
  if (node_type_info.display_name !=
          orc::plugins::field_map::kStageDisplayName ||
      node_type_info.menu_category !=
          orc::plugins::field_map::kStageMenuCategory ||
      ...
      node_type_info.compatible_formats !=
          orc::plugins::field_map::kStageCompatibleFormats ||
      node_type_info.sink_category !=
          orc::plugins::field_map::kStageSinkCategory) {
    if (error_message) {
      *error_message =
  ...
  if (!register_stage(context, orc::plugins::field_map::kStageName,
                      &create_stage)) {
    if (error_message) {
