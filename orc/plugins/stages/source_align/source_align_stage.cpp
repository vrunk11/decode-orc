...
class AlignedSourceRepresentation : public VideoFieldRepresentationWrapper {
 public:
  AlignedSourceRepresentation(
      std::shared_ptr<const VideoFieldRepresentation> source, FieldID offset,
      size_t source_index)
      : VideoFieldRepresentationWrapper(
            source,
            ArtifactID("aligned_source_" + std::to_string(source_index) +
                       "_offset_" + std::to_string(offset.value())),
            Provenance{
                "source_align",
                "1.0",
  ...
  for (size_t i = 0; i < sources.size() && i < offsets.size(); ++i) {
    ...
    if (parity_hint.has_value()) {
      ...
      if (!target_is_first_field.has_value()) {
        ...
        ORC_LOG_DEBUG(
            "Field order enforcement: adjusted source {} offset from {} to {} "
            "to match parity",
...
int32_t SourceAlignStage::get_frame_number_from_vbi(
    const VideoFieldRepresentation& source, FieldID field_id) const {
  (void)source;
  (void)field_id;
...
std::vector<FieldID> SourceAlignStage::find_alignment_offsets(
  ...
  for (size_t src_idx = 0; src_idx < sources.size(); ++src_idx) {
    ...
    ORC_LOG_DEBUG("  Source {}: scanning {} fields (range {}-{})", src_idx,
                  source->field_count(), range.start.value(),
                  range.end.value() - 1);
  ...
  std::vector<FieldID> alignment_offsets(
      sources.size());  // Default constructor creates invalid FieldIDs
  ...
  for (const auto& [frame_num, locations] : frame_map) {
    ...
    if (all_present) {
      ...
      ORC_LOG_INFO("  Found first common frame: VBI frame #{}",
                   first_common_frame);
      for (size_t i = 0; i < sources.size(); ++i) {
        ORC_LOG_INFO("    Source {}: starts at field_id {}", i,
  ...
  if (first_common_frame < 0) {
    ...
    ORC_LOG_WARN(
        "  This may indicate sources are from different discs or have no VBI "
        "data");
...
std::vector<ArtifactPtr> SourceAlignStage::execute(
  ...
  ORC_LOG_DEBUG("SourceAlignStage: Processing {} input source(s)",
                inputs.size());
  ...
  for (const auto& input : inputs) {
    ...
    if (!source) {
      throw DAGExecutionError(
          "SourceAlignStage input must be VideoFieldRepresentation");
    }
  ...
  if (!alignment_map_.empty()) {
    ...
    if (alignment_entries.empty()) {
      throw DAGExecutionError("Invalid alignment map specification: " +
                              alignment_map_);
    }

    // Build offsets array from alignment map
    // Initialize all to INVALID (excluded by default)
    offsets.resize(sources.size(),
                   FieldID());  // Default constructor creates INVALID
    ...
    for (size_t i = 0; i < offsets.size(); ++i) {
      ORC_LOG_DEBUG("  Input {}: {}", i + 1,
                    offsets[i].is_valid()
                        ? "offset = " + std::to_string(offsets[i].value())
                        : "EXCLUDED");
    }
  } else {
  ...
  for (size_t i = 0; i < sources.size(); ++i) {
    ...
    if (offsets[i].value() == 0) {
      ...
      ORC_LOG_DEBUG(
          "  Source {}: no alignment needed (offset=0), passing through "
          "unchanged",
          i);
    } else {
...
std::vector<ParameterDescriptor> SourceAlignStage::get_parameter_descriptors(
  ...
  return {
      ParameterDescriptor{
          ...
          "Format: input_id+offset for each input. Empty = auto-detect from "
          "VBI.",
      ...
      ParameterDescriptor{"enforceFieldOrder", "Enforce Field Order",
                          "When enabled, ensures the first output field is "
                          "always a first field "
                          "(adds extra field if needed). Recommended for "
                          "proper interlaced output.",
                          ParameterType::BOOL,
                          ParameterConstraints{
                              std::nullopt,          // no min
                              std::nullopt,          // no max
...
std::optional<StageReport> SourceAlignStage::generate_report() const {
  ...
  if (input_sources_.empty() || alignment_offsets_.empty()) {
    ...
    report.items.push_back(
        {"Info", "Execute the DAG to see alignment details"});
    return report;
  }
  ...
  for (size_t i = 0; i < input_sources_.size(); ++i) {
    ...
    if (!offset.is_valid()) {
      ...
      report.items.push_back({source_label + " Input Range",
                              std::to_string(range.start.value()) + "-" +
                                  std::to_string(range.end.value())});
      report.items.push_back(
          {source_label + " Input Fields", std::to_string(input_count)});
    } else {
      auto range = source->field_range();
      size_t input_count = source->field_count();
      ...
      report.items.push_back(
          {source_label + " Input Fields", std::to_string(input_count)});
      report.items.push_back(
          {source_label + " Alignment Offset", std::to_string(dropped)});
      report.items.push_back(
          {source_label + " Dropped Fields", std::to_string(dropped)});
      report.items.push_back(
          {source_label + " Output Fields", std::to_string(output_count)});

      // Add VBI frame number at alignment point if available
      int32_t vbi_frame = get_frame_number_from_vbi(*source, offset);
      if (vbi_frame >= 0) {
        report.items.push_back({source_label + " First Common VBI Frame",
                                std::to_string(vbi_frame)});
      }
    }
  ...
  report.metrics["included_sources"] =
      static_cast<int64_t>(input_sources_.size() - excluded_count);
  ...
  for (size_t i = 0; i < cached_outputs_.size(); ++i) {
    ...
    if (output && output->field_count() > 0) {
      ...
      options.push_back({"source_" + std::to_string(i),
                         ...
                         false,  // is_rgb
                         width, height, output->field_count(), dar});
    }
  }
