...
ORC_STAGE_PLUGIN_EXPORT bool orc_register_stage_plugin(
  ...
  if (node_type_info.display_name !=
          orc::plugins::source_align::kStageDisplayName ||
      node_type_info.menu_category !=
          orc::plugins::source_align::kStageMenuCategory ||
      node_type_info.type != orc::plugins::source_align::kStageNodeType ||
      node_type_info.min_inputs !=
          orc::plugins::source_align::kStageMinInputs ||
      node_type_info.max_inputs !=
          orc::plugins::source_align::kStageMaxInputs ||
      node_type_info.min_outputs !=
          orc::plugins::source_align::kStageMinOutputs ||
      node_type_info.max_outputs !=
          orc::plugins::source_align::kStageMaxOutputs ||
      node_type_info.compatible_formats !=
          orc::plugins::source_align::kStageCompatibleFormats ||
      node_type_info.sink_category !=
          orc::plugins::source_align::kStageSinkCategory) {
    if (error_message) {
      *error_message =
  ...
  if (!register_stage(context, orc::plugins::source_align::kStageName,
                      &create_stage)) {
    if (error_message) {
