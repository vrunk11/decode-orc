        ...
        try {
            ...
            if (stage_services_) {
                ...
                public:
                    ...
                    bool open(const std::string& filepath, std::ios::openmode mode [[maybe_unused]] = std::ios::binary | std::ios::trunc) override
                    {
                        path_ = filepath;
                        return impl_ && impl_->open(filepath);
                    }

                    void write(const uint8_t* data, size_t count) override { if (impl_) impl_->write(data, count); }
                    void write(const std::vector<uint8_t>& data) override { if (impl_) impl_->write(data); }
                    void flush() override { if (impl_) impl_->flush(); }
                    void close() override { if (impl_) impl_->close(); }
                ...
                auto writer8 = stage_services_->create_buffered_file_writer_uint8(1 * 1024 * 1024);
                if (writer8) {
            ...
            std::shared_ptr<DaphneVBIWriterUtil> daphne_vbi_writer_util = std::make_shared<DaphneVBIWriterUtil>();
