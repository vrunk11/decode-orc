...
CCSinkStage::ParsedConfig CCSinkStage::parse_config(
    ...
    if (output_path_it == parameters.end() || !std::holds_alternative<std::string>(output_path_it->second)) {
    ...
    if (format_it != parameters.end() && std::holds_alternative<std::string>(format_it->second)) {
    ...
    if (write_csv_it != parameters.end() && std::holds_alternative<bool>(write_csv_it->second)) {
...
bool CCSinkStage::trigger(
    ...
    try {
        ...
        const CCExportResult export_result = deps->export_cc(vfr.get(), observation_context, options);

        is_processing_.store(false);

        if (!export_result.success) {
            ORC_LOG_ERROR("CC sink error: {}", export_result.message.empty() ? "Failed to export closed captions" : export_result.message);
            return false;
        }

        ORC_LOG_INFO(
            "Closed caption export completed successfully: {} CC frames",
