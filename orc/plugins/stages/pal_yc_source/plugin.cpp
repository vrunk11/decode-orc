...
ORC_STAGE_PLUGIN_EXPORT bool orc_register_stage_plugin(
  ...
  if (node_type_info.display_name !=
          orc::plugins::pal_yc_source::kStageDisplayName ||
      node_type_info.menu_category !=
          orc::plugins::pal_yc_source::kStageMenuCategory ||
      node_type_info.type != orc::plugins::pal_yc_source::kStageNodeType ||
      node_type_info.min_inputs !=
          orc::plugins::pal_yc_source::kStageMinInputs ||
      node_type_info.max_inputs !=
          orc::plugins::pal_yc_source::kStageMaxInputs ||
      node_type_info.min_outputs !=
          orc::plugins::pal_yc_source::kStageMinOutputs ||
      node_type_info.max_outputs !=
          orc::plugins::pal_yc_source::kStageMaxOutputs ||
      node_type_info.compatible_formats !=
          orc::plugins::pal_yc_source::kStageCompatibleFormats ||
      node_type_info.sink_category !=
          orc::plugins::pal_yc_source::kStageSinkCategory) {
    if (error_message) {
      *error_message =
  ...
  if (!register_stage(context, orc::plugins::pal_yc_source::kStageName,
                      &create_stage)) {
    if (error_message) {
