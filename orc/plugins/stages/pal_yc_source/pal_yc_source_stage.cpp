...
std::vector<ArtifactPtr> PALYCSourceStage::execute(
  ...
  if (y_path_it == parameters.end() ||
    ...
    ORC_LOG_DEBUG(
        "PAL_YC_Source: No y_path configured, returning empty output");
    return {};
  }
  ...
  if (c_path_it == parameters.end() ||
    ...
    ORC_LOG_DEBUG(
        "PAL_YC_Source: No c_path configured, returning empty output");
    return {};
  }
  ...
  if (cached_representation_ && cached_y_path_ == y_path &&
    ...
    ORC_LOG_DEBUG("PAL_YC_Source: Using cached representation for {} + {}",
                  y_path, c_path);
  ...
  try {
    ...
    if (video_params->system != VideoSystem::PAL &&
      ...
      throw UserDataError(
          "YC files are not PAL or PAL-M format. Use 'Add NTSC YC Source' for "
          "NTSC files.");
    }
    ...
    throw UserDataError(std::string("Failed to load PAL-family YC files '") +
                        y_path + "' + '" + c_path + "': " + e.what());
  }
}

std::vector<ParameterDescriptor> PALYCSourceStage::get_parameter_descriptors(
...
bool PALYCSourceStage::set_parameters(
