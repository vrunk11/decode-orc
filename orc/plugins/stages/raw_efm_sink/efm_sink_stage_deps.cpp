...
namespace orc
{
    void RawEFMSinkStageDeps::init(TriggerProgressCallback progress_callback, std::atomic<bool>* cancel_requested)
    {
        ...
        const std::string& output_path)
    {
        ...
        ORC_LOG_INFO("RawEFMSinkDeps: Successfully wrote {} t-values to {}", tvalues_written, output_path);
        ORC_LOG_DEBUG("RawEFMSinkDeps: Expected t-values: {}, Actual t-values: {}, Match: {}",
                      total_tvalues, tvalues_written, total_tvalues == tvalues_written ? "YES" : "NO");

        if (invalid_tvalue_count > 0) {
            ORC_LOG_WARN("RawEFMSinkDeps: Found {} invalid t-values (outside range [3, 11])",
                         invalid_tvalue_count);
        }

        return {true, tvalues_written, "Success: " + std::to_string(tvalues_written) + " t-values written"};
    }
} // namespace orc
