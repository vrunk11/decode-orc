    ...
    const char** error_message)
{
    ...
    if (node_type_info.display_name != orc::plugins::stacker::kStageDisplayName ||
        node_type_info.menu_category != orc::plugins::stacker::kStageMenuCategory ||
        ...
        node_type_info.compatible_formats != orc::plugins::stacker::kStageCompatibleFormats ||
        node_type_info.sink_category != orc::plugins::stacker::kStageSinkCategory) {
        if (error_message) {
    ...
    if (!register_stage(context, orc::plugins::stacker::kStageName, &create_stage)) {
        if (error_message) {
