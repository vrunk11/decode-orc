...
class StackerStage : public DAGStage,
 ...
 public:
  ...
  NodeTypeInfo get_node_type_info() const override {
    return NodeTypeInfo{NodeType::MERGER,
                        "stacker",
                        "Stacker",
                        "Combine multiple TBC sources by stacking fields for "
                        "superior output quality (1 input = passthrough)",
                        1,
                        16,  // 1 to 16 inputs
                        1,
  ...
  size_t required_input_count() const override {
    return 1;
  }  // At least 1 input (passthrough mode)
 ...
 private:
  // Stacking parameters
  int32_t m_mode;  // Stacking mode (-1=Auto, 0=Mean, 1=Median, 2=Smart Mean,
  ...
  int32_t m_thread_count;  // Number of threads for parallel processing (0=auto,
                           // max=hardware concurrency)
  AudioStackingMode
      m_audio_stacking_mode;            // Audio stacking mode (default: mean)
  EFMStackingMode m_efm_stacking_mode;  // EFM stacking mode (default: mean)

  // Store parameters for inspection
  std::map<std::string, ParameterValue> parameters_;

  // Cache the stacked representation to preserve LRU caches across execute()
  // calls
   ...
   *
   * @param field_id Field ID to stack (same ID used for all sources - alignment
   * done by field_map stages)
  ...
  /**
   * @brief Stack a single YC field from multiple sources (separate Y and C
   * channels)
   *
   * @param field_id Field ID to stack
   * @param sources Input field representations (must all have separate
   * channels)
   ...
   */
  uint16_t stack_mode(const std::vector<uint16_t>& values,
   ...
   * @param output_samples Output sample buffer (shared, each thread writes to
   * its own lines)
  ...
  /**
   * @brief Process a range of lines for YC sources with channel-consistent
   * corrections
   *
   ...
   */
  std::vector<uint16_t> diff_dod(const std::vector<uint16_t>& input_values,
   ...
   * @param sources Input field representations
   * @param best_source_index Index of best source (used when audio stacking
   * disabled)
   ...
   * @param field_id Field ID
   * @param sources Input field representations
   * @param best_source_index Index of best source (used when EFM stacking
   * disabled)
