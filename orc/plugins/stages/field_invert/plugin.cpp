...
ORC_STAGE_PLUGIN_EXPORT bool orc_register_stage_plugin(
  ...
  if (node_type_info.display_name !=
          orc::plugins::field_invert::kStageDisplayName ||
      node_type_info.menu_category !=
          orc::plugins::field_invert::kStageMenuCategory ||
      node_type_info.type != orc::plugins::field_invert::kStageNodeType ||
      node_type_info.min_inputs !=
          orc::plugins::field_invert::kStageMinInputs ||
      node_type_info.max_inputs !=
          orc::plugins::field_invert::kStageMaxInputs ||
      node_type_info.min_outputs !=
          orc::plugins::field_invert::kStageMinOutputs ||
      node_type_info.max_outputs !=
          orc::plugins::field_invert::kStageMaxOutputs ||
      node_type_info.compatible_formats !=
          orc::plugins::field_invert::kStageCompatibleFormats ||
      node_type_info.sink_category !=
          orc::plugins::field_invert::kStageSinkCategory) {
    if (error_message) {
      *error_message =
  ...
  if (!register_stage(context, orc::plugins::field_invert::kStageName,
                      &create_stage)) {
    if (error_message) {
