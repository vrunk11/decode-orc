...
public:
    ...
    virtual bool validate_input_file(const std::string& input_path, std::string& error_message) const = 0;
    virtual std::optional<CVBSMetadataRecord> load_metadata(const std::string& meta_path, std::string& error_message) const = 0;

    // Returns the total number of 16-bit words in the file without loading it into memory.
    virtual std::optional<size_t> get_input_word_count(const std::string& input_path, std::string& error_message) const = 0;

    // Reads exactly word_count 16-bit words starting at word_offset from the file.
    virtual bool read_input_words_at(const std::string& input_path, size_t word_offset, size_t word_count, std::vector<uint16_t>& out_words, std::string& error_message) const = 0;
};
 ...
 *
 * **Signal State Constraint:** Only `STANDARD_TBC_LOCKED` signal state is supported.
 * Files with other signal states (e.g., STANDARD_TBC_UNLOCKED, STANDARD_RAW) will
 * be rejected with clear validation errors.
 *
 * **Sample Encoding Support:**
 * - `CVBS_TPG21_4FSC` - Device-encoded composite (requires offset/scale reversal)
 ...
 * Each field carries stable identity metadata (frame index, field-in-frame index,
 * standard context) for deterministic processing.
 *
 * **Parameters:**
 */
class FixedFormatCVBSSourceStage : public DAGStage, public ParameterizedStage, public PreviewableStage {
public:
    ...
    void set_deps_override(std::shared_ptr<ICVBSSourceStageDeps> deps) { deps_override_ = std::move(deps); }
    ...
    NodeTypeInfo get_node_type_info() const override {
        return NodeTypeInfo{
            ...
            0, 0,        // No inputs
            1, UINT32_MAX,  // Many outputs (fields)
...
private:
    ...
    std::string sample_encoding_;  // "CVBS_TPG21_4FSC" or "CVBS_U16_4FSC" (manual mode)
...
public:
    explicit PALCVBSSourceStage(std::shared_ptr<ICVBSSourceStageDeps> deps = nullptr);
    ~PALCVBSSourceStage() override = default;
};

class NTSCCVBSSourceStage final : public FixedFormatCVBSSourceStage {
public:
    explicit NTSCCVBSSourceStage(std::shared_ptr<ICVBSSourceStageDeps> deps = nullptr);
