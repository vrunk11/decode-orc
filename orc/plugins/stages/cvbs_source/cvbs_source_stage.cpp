...
// Helper: Determine sample count for a PAL line at given field-line index (0-indexed within field).
// PAL spec: exactly 4 lines per frame have 1136 samples, remainder use 1135.
// Pattern per field: line_index in {155, 311} for even field; {156, 312} for odd field.
inline size_t pal_line_sample_count(size_t field_line_index, bool is_odd_field)
{
    if (is_odd_field) {
...
// Calculate total samples in a PAL field accounting for 1136-sample fractional lines.
...
std::optional<size_t> detect_sync_start_sample(
    ...
    int32_t sync_tip_ire_16b)
{
    ...
    const int32_t threshold = sync_tip_ire_16b + ((blanking_ire_16b - sync_tip_ire_16b) / 2);
...
size_t detect_global_sync_start_sample(
    const std::vector<uint16_t>& normalized_words,
    const CVBSGeometry& geometry,
    size_t full_frame_count)
{
    const size_t frame_samples = geometry.samples_per_line * (geometry.first_field_lines + geometry.second_field_lines);
    std::vector<size_t> detections;
    detections.reserve(std::min(full_frame_count, static_cast<size_t>(8)) *
                       (geometry.first_field_lines + geometry.second_field_lines));

    const size_t probe_frame_count = std::min(full_frame_count, static_cast<size_t>(8));

    for (size_t frame_index = 0; frame_index < probe_frame_count; ++frame_index) {
        const size_t frame_offset = frame_index * frame_samples;
        const size_t lines_in_frame = geometry.first_field_lines + geometry.second_field_lines;

        for (size_t line_index = 0; line_index < lines_in_frame; ++line_index) {
            const size_t line_offset = frame_offset + (line_index * geometry.samples_per_line);
...
// Rotate uniform-length lines to sync start (used for NTSC and standard PAL processing).
void rotate_field_lines_to_sync_start(
    ...
    int32_t sync_tip_ire_16b)
{
    ...
    for (size_t line_index = 0; line_index < line_count; ++line_index) {
        ...
        if (detected_sync.has_value()) {
            ...
            if (shift > 0) {
                auto line_begin = field_samples.begin() + static_cast<std::ptrdiff_t>(offset);
                std::rotate(line_begin, line_begin + static_cast<std::ptrdiff_t>(shift), line_begin + static_cast<std::ptrdiff_t>(line_length));
            }
        }
    }
}
...
std::optional<size_t> detect_shared_frame_sync_start_sample(
    ...
    const std::vector<uint16_t>& second_field_samples,
    size_t line_length,
    int32_t blanking_ire_16b,
    int32_t sync_tip_ire_16b)
{
    ...
    std::vector<size_t> detections;
    detections.reserve((first_field_samples.size() + second_field_samples.size()) / line_length);

    const auto append_detections = [&](const std::vector<uint16_t>& field_samples) {
        const size_t line_count = field_samples.size() / line_length;
        for (size_t line_index = 0; line_index < line_count; ++line_index) {
            const size_t offset = line_index * line_length;
            const uint16_t* line_ptr = field_samples.data() + offset;
    ...
    int32_t sync_tip_ire_16b)
{
    ...
    for (size_t field_line_index = 0; field_line_index < max_field_lines; ++field_line_index) {
        const size_t actual_line_length = pal_line_sample_count(field_line_index, is_odd_field);
...
std::optional<size_t> detect_shared_pal_frame_sync_start_sample_variable(
    ...
    const std::vector<uint16_t>& even_field_samples,
    int32_t blanking_ire_16b,
    int32_t sync_tip_ire_16b)
{
    std::vector<size_t> detections;
...
void rotate_field_lines_by_fixed_shift(
    std::vector<uint16_t>& field_samples,
    size_t line_length,
    size_t shift)
{
...
void rotate_pal_field_lines_by_fixed_shift(
    std::vector<uint16_t>& field_samples,
    bool is_odd_field,
    size_t shift)
{
    ...
    for (size_t field_line_index = 0; field_line_index < max_field_lines; ++field_line_index) {
        ...
        if (normalized_shift > 0) {
            auto line_begin = field_samples.begin() + static_cast<std::ptrdiff_t>(sample_offset);
            std::rotate(
                line_begin,
                line_begin + static_cast<std::ptrdiff_t>(normalized_shift),
    ...
    size_t output_samples)
{
    ...
    if (error != nullptr) {
        throw std::runtime_error(std::string("CVBS PAL resampler init failed: ") + error);
    }
    ...
    if (error != nullptr) {
        throw std::runtime_error(std::string("CVBS PAL resampler process failed: ") + error);
    }

    if (output_done < output_samples && output_done > 0) {
        const float last = output_float[output_done - 1];
        std::fill(output_float.begin() + static_cast<std::ptrdiff_t>(output_done), output_float.end(), last);
    }
...
// Rotate PAL field lines accounting for variable line lengths (fractional 4fsc lines).
// Each line gets rotated independently based on its actual sample count (1135 or 1136).
void rotate_pal_field_lines_to_sync_start(
    std::vector<uint16_t>& field_samples,
    bool is_odd_field,
    int32_t blanking_ire_16b,
    int32_t sync_tip_ire_16b)
{
    ...
    for (size_t field_line_index = 0; field_line_index < max_field_lines; ++field_line_index) {
        ...
        if (detected_sync.has_value()) {
            ...
            if (shift > 0) {
                auto line_begin = field_samples.begin() + static_cast<std::ptrdiff_t>(sample_offset);
                std::rotate(line_begin, line_begin + static_cast<std::ptrdiff_t>(shift), line_begin + static_cast<std::ptrdiff_t>(actual_line_length));
            }
        }
...
int32_t normalize_sample_to_internal_domain(uint16_t raw_word, const std::string& sample_encoding, size_t& clamped_low_count)
{
    int32_t value_10bit = 0;

    if (sample_encoding == "CVBS_TPG21_4FSC") {
        const int16_t signed_word = static_cast<int16_t>(raw_word);
        const auto decoded = static_cast<int32_t>(std::lround(static_cast<double>(signed_word) / 64.0));
...
public:
    ...
    std::optional<FieldDescriptor> get_descriptor(FieldID id) const override
    {
        ...
        descriptor.height = is_first_field ? geometry_.first_field_lines : geometry_.second_field_lines;
        return descriptor;
    }
...
private:
    ...
    DecodedFrame decode_frame(size_t frame_index) const
    {
        ...
        if (!deps_->read_input_words_at(input_path_, frame_word_offset, frame_samples_, raw_frame_words, read_error)) {
        ...
        if (raw_frame_words.size() != frame_samples_) {
            throw std::runtime_error(
                "Short CVBS frame read at frame " + std::to_string(frame_index) +
                " from payload '" + input_path_ + "'");
        }
        ...
        for (uint16_t raw_word : raw_frame_words) {
            normalized_words.push_back(static_cast<uint16_t>(
                normalize_sample_to_internal_domain(raw_word, sample_encoding_, clamped_low_count)));
        }
        ...
        if (geometry_.system == VideoSystem::PAL) {
            std::vector<uint16_t> first_field(
                ...
                normalized_words.begin() + static_cast<std::ptrdiff_t>(first_field_samples_));
            std::vector<uint16_t> second_field(
                normalized_words.begin() + static_cast<std::ptrdiff_t>(first_field_samples_),
                normalized_words.begin() + static_cast<std::ptrdiff_t>(first_field_samples_ + second_field_samples_));

            auto& odd_field = first_field;
            auto& even_field = second_field;

            const auto shared_sync = detect_shared_pal_frame_sync_start_sample_variable(
                odd_field,
                even_field,
                geometry_.blanking_ire_16b,
            ...
            frame_samples_aligned.insert(
                frame_samples_aligned.end(),
                first_field.begin(),
                first_field.end());
            frame_samples_aligned.insert(
                frame_samples_aligned.end(),
            ...
            const size_t pal_uniform_frame_samples =
                (geometry_.first_field_lines + geometry_.second_field_lines) * geometry_.samples_per_line;
            std::vector<uint16_t> frame_uniform =
                resample_sequence_sinc_soxr(frame_samples_aligned, pal_uniform_frame_samples);

            const size_t odd_uniform_samples = geometry_.first_field_lines * geometry_.samples_per_line;
            result.first_field.assign(
                frame_uniform.begin(),
                frame_uniform.begin() + static_cast<std::ptrdiff_t>(odd_uniform_samples));
...
public:
    bool validate_input_file(const std::string& input_path, std::string& error_message) const override
    {
        namespace fs = std::filesystem;
        ...
        if (!fs::is_regular_file(input_path, ec)) {
            error_message = "CVBS source path is not a regular file: '" + input_path + "'";
            return false;
        }
        ...
        if (!input_stream.good()) {
            error_message = "Failed to inspect CVBS source file size: '" + input_path + "'";
            return false;
        }
    ...
    std::optional<CVBSMetadataRecord> load_metadata(const std::string& meta_path, std::string& error_message) const override
    {
        namespace fs = std::filesystem;
        std::error_code ec;
        ...
        if (!fs::is_regular_file(meta_path, ec)) {
            error_message = "Metadata path is not a regular file: '" + meta_path + "'";
            return std::nullopt;
        }

        sqlite3* db = nullptr;
        const int open_result = sqlite3_open_v2(meta_path.c_str(), &db, SQLITE_OPEN_READONLY, nullptr);
        if (open_result != SQLITE_OK) {
            error_message = "Failed to open metadata file '" + meta_path + "': " +
                            std::string(db != nullptr ? sqlite3_errmsg(db) : "unknown sqlite error");
            if (db != nullptr) {
                sqlite3_close(db);
            }
            return std::nullopt;
        ...
        if (prepare_result != SQLITE_OK) {
            error_message = "Failed to query cvbs_file metadata from '" + meta_path + "': " + sqlite3_errmsg(db);
            sqlite3_close(db);
            return std::nullopt;
        }
        ...
        if (step_result != SQLITE_ROW) {
            error_message = "Metadata file '" + meta_path + "' does not contain a cvbs_file row";
        ...
        record.preset = preset != nullptr ? reinterpret_cast<const char*>(preset) : "";
        record.sample_encoding_preset = sample_encoding != nullptr ? reinterpret_cast<const char*>(sample_encoding) : "";
        record.signal_state_preset = signal_state != nullptr ? reinterpret_cast<const char*>(signal_state) : "";
        record.signal_type = signal_type != nullptr ? reinterpret_cast<const char*>(signal_type) : "";
    ...
    std::optional<size_t> get_input_word_count(const std::string& input_path, std::string& error_message) const override
    {
        std::ifstream input_stream(input_path, std::ios::binary);
        if (!input_stream.is_open()) {
        ...
        if ((size_bytes % 2) != 0) {
            error_message = "CVBS payload file size is not 16-bit aligned: '" + input_path + "'";
            return std::nullopt;
        }
    ...
    bool read_input_words_at(const std::string& input_path, size_t word_offset, size_t word_count,
                             std::vector<uint16_t>& out_words, std::string& error_message) const override
    {
        std::ifstream input_stream(input_path, std::ios::binary);
        if (!input_stream.is_open()) {
            error_message = "Failed to open CVBS payload file for reading: '" + input_path + "'";
            return false;
        }

        const std::streamoff byte_offset = static_cast<std::streamoff>(word_offset) * 2;
        input_stream.seekg(byte_offset, std::ios::beg);
        if (!input_stream.good()) {
            error_message = "Failed to seek in CVBS payload file: '" + input_path + "'";
            return false;
        }
        ...
        if (!input_stream.good() && !input_stream.eof()) {
            error_message = "Failed to read CVBS payload words from file: '" + input_path + "'";
            return false;
        }
...
PALCVBSSourceStage::PALCVBSSourceStage(std::shared_ptr<ICVBSSourceStageDeps> deps)
    : FixedFormatCVBSSourceStage(kPALIdentity, std::move(deps))
{
}

NTSCCVBSSourceStage::NTSCCVBSSourceStage(std::shared_ptr<ICVBSSourceStageDeps> deps)
    ...
    ObservationContext& observation_context)
{
    ...
    if (!inputs.empty()) {
        throw std::runtime_error(std::string(identity_.stage_name) + " stage should have no inputs");
    }
    ...
    if (input_path_it == parameters.end() || std::get<std::string>(input_path_it->second).empty()) {
        ...
        ORC_LOG_DEBUG("{}: No input_path configured, returning empty output", identity_.stage_name);
        return {};
    }
    ...
    if (use_metadata) {
        ...
        std::string validation_error = validate_metadata_mode(
            input_path,
            meta_path,
    ...
    const auto total_words_opt = deps_override_->get_input_word_count(input_path, word_count_error);
    if (!total_words_opt.has_value()) {
        throw UserDataError("Failed to inspect CVBS payload '" + input_path + "': " + word_count_error);
    }
    ...
    if (geometry.system == VideoSystem::PAL) {
        frame_samples = pal_field_total_samples(true) + pal_field_total_samples(false);
    } else {
        frame_samples = geometry.samples_per_line * (geometry.first_field_lines + geometry.second_field_lines);
    }
    ...
    if (full_frame_count == 0) {
        throw UserDataError(
            "CVBS payload '" + input_path + "' is too short for one complete " + resolved_video_standard +
            " frame at 4fsc geometry");
    }

    const int32_t active_video_start = geometry.active_video_start;
    const int32_t active_video_end = std::min(
    ...
    if (geometry.system == VideoSystem::PAL) {
        ...
        first_field_samples = geometry.samples_per_line * geometry.first_field_lines;
        second_field_samples = geometry.samples_per_line * geometry.second_field_lines;
    }

    ORC_LOG_DEBUG(
        "{}: Horizontal sync alignment per-line (active {}..{})",
    ...
    if (trailing_samples > 0) {
        ORC_LOG_WARN(
            "{}: Dropped {} trailing samples from '{}' that do not form a complete frame",
            identity_.stage_name,
            trailing_samples,
            input_path);
    }
    ...
    source_parameters.field_width = static_cast<int32_t>(geometry.samples_per_line);
    source_parameters.field_height = static_cast<int32_t>(calculate_padded_field_height(geometry.system));
    source_parameters.number_of_sequential_fields = static_cast<int32_t>(full_frame_count * 2);
    ...
    auto representation = std::make_shared<CVBSDecodedFieldRepresentation>(
        ...
        full_frame_count,
        source_parameters,
        ArtifactID(std::string(identity_.stage_name) + ":" + input_path + ":" + resolved_sample_encoding),
        std::move(provenance));
...
std::vector<ParameterDescriptor> FixedFormatCVBSSourceStage::get_parameter_descriptors(VideoSystem project_format, SourceType source_type) const
{
    ...
    // input_path parameter
    {
        ...
        desc.description = "Path to the CVBS (Composite Video Baseband Signal) data file. "
    ...
    // use_metadata parameter
    {
        ...
        desc.description = "Enable metadata-driven mode: reads sample encoding from .meta sidecar file and validates "
    ...
    // sample_encoding parameter (manual mode only)
    {
        ...
        desc.description = "Sample encoding preset for manual mode (ignored if use_metadata is enabled). "
...
std::map<std::string, ParameterValue> FixedFormatCVBSSourceStage::get_parameters() const
{
...
bool FixedFormatCVBSSourceStage::set_parameters(const std::map<std::string, ParameterValue>& params)
{
...
std::vector<PreviewOption> FixedFormatCVBSSourceStage::get_preview_options() const
{
    ...
    if (field_count >= 2) {
        ...
        options.push_back(PreviewOption{
            "frame", "Frame (Clamped)", false, width, height * 2, pair_count, dar_correction});

        options.push_back(PreviewOption{
            "frame_raw", "Frame (Raw)", false, width, height * 2, pair_count, dar_correction});
    }
...
std::optional<StageReport> FixedFormatCVBSSourceStage::generate_report() const
{
    ...
    report.items.push_back({"Mode", use_metadata_ ? "Metadata-driven" : "Manual"});
    report.items.push_back({"Video Standard", identity_.fixed_video_standard});

    if (!use_metadata_) {
        report.items.push_back({"Sample Encoding", sample_encoding_.empty() ? "Not set" : sample_encoding_});
    }
    ...
    std::string& resolved_sample_encoding) const
{
    ...
    const auto metadata_record = deps_override_->load_metadata(meta_path, metadata_error);
    if (!metadata_record.has_value()) {
        return "Failed to load CVBS metadata from '" + meta_path + "': " + metadata_error;
    }

    const auto& record = *metadata_record;

    if (record.preset != identity_.fixed_video_standard) {
        return "Unsupported metadata preset '" + record.preset +
               "' in '" + meta_path + "'. This stage requires: " + std::string(identity_.fixed_video_standard);
    }

    if (record.sample_encoding_preset != "CVBS_U16_4FSC" && record.sample_encoding_preset != "CVBS_TPG21_4FSC") {
        return "Unsupported metadata sample_encoding_preset '" + record.sample_encoding_preset +
               "' in '" + meta_path + "'. Supported encodings: CVBS_U16_4FSC, CVBS_TPG21_4FSC";
    }
    resolved_sample_encoding = record.sample_encoding_preset;

    if (record.signal_state_preset != "STANDARD_TBC_LOCKED") {
        return "Unsupported metadata signal_state_preset '" + record.signal_state_preset +
               "' in '" + meta_path + "'. CVBS source requires: STANDARD_TBC_LOCKED";
    }
...
std::string FixedFormatCVBSSourceStage::validate_manual_mode(const std::string& sample_encoding) const
{
    // Validate sample_encoding
    if (sample_encoding != "CVBS_U16_4FSC" && sample_encoding != "CVBS_TPG21_4FSC") {
        return "Invalid sample_encoding '" + sample_encoding +
               "'. Supported: CVBS_U16_4FSC, CVBS_TPG21_4FSC";
    }

    // Signal state is implicitly STANDARD_TBC_LOCKED (validated in Phase 2 for metadata mode)
    return "";
}
