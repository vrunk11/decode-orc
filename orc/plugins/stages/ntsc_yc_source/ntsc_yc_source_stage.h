 ...
 */
class NTSCYCSourceStage : public DAGStage,
                          public ParameterizedStage,
                          public PreviewableStage {
 public:
  explicit NTSCYCSourceStage(
      std::shared_ptr<INTSCYCSourceLoader> loader = nullptr);
  ...
  NodeTypeInfo get_node_type_info() const override {
    return NodeTypeInfo{NodeType::SOURCE,
                        "NTSC_YC_Source",
                        "NTSC YC Source",
                        "NTSC YC input source - loads separate Y and C TBC "
                        "files (color-under formats like VHS)",
                        0,
                        0,  // No inputs
                        1,
