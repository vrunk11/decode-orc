...
class DefaultNTSCYCSourceLoader final : public INTSCYCSourceLoader {
 public:
  std::shared_ptr<VideoFieldRepresentation> load(
    ...
    return source_loader_shared::load_tbc_yc(y_path, c_path, db_path, pcm_path,
                                             efm_path, ac3rf_path);
  }
};
...
NTSCYCSourceStage::NTSCYCSourceStage(
    std::shared_ptr<INTSCYCSourceLoader> loader)
...
NTSCYCSourceStage::load_representation(const std::string& y_path,
...
std::vector<ArtifactPtr> NTSCYCSourceStage::execute(
  ...
  if (y_path_it == parameters.end() ||
      std::get<std::string>(y_path_it->second).empty()) {
    // No file path configured - return empty artifact (0 fields)
    ORC_LOG_DEBUG(
        "NTSC_YC_Source: No y_path configured, returning empty output");
    return {};
  }
  ...
  if (c_path_it == parameters.end() ||
      std::get<std::string>(c_path_it->second).empty()) {
    // No C path configured
    ORC_LOG_DEBUG(
        "NTSC_YC_Source: No c_path configured, returning empty output");
    return {};
  }
  ...
  if (db_path_it != parameters.end() &&
      !std::get<std::string>(db_path_it->second).empty()) {
  ...
  // Check cache
  if (cached_representation_ && cached_y_path_ == y_path &&
      cached_c_path_ == c_path) {
    ORC_LOG_DEBUG("NTSC_YC_Source: Using cached representation for {} + {}",
                  y_path, c_path);
  ...
  try {
    auto yc_representation = load_representation(
    ...
    ORC_LOG_DEBUG("  Fields: {} ({}x{} pixels)",
                  video_params->number_of_sequential_frames * 2,
                  video_params->frame_width_nominal,
                  calculate_padded_field_height(video_params->system));

    // Check system
    if (video_params->system != VideoSystem::NTSC) {
      throw UserDataError(
          "YC files are not NTSC format. Use 'Add PAL YC Source' for PAL "
          "files.");
    }
    ...
    throw UserDataError(std::string("Failed to load NTSC YC files '") + y_path +
                        "' + '" + c_path + "': " + e.what());
  }
}

std::vector<ParameterDescriptor> NTSCYCSourceStage::get_parameter_descriptors(
  ...
  // y_path parameter
  {
    ...
    desc.constraints.required =
        false;  // Optional - source provides 0 fields until path is set
  ...
  // db_path parameter
  {
    ...
    desc.description =
        "Path to the .tbc.db metadata file (defaults to Y file path + .db)";
    desc.type = ParameterType::FILE_PATH;
  ...
  // pcm_path parameter
  {
    ...
    desc.description =
        "Path to the analogue audio .pcm file (raw 16-bit stereo PCM at "
        "44.1kHz)";
    desc.type = ParameterType::FILE_PATH;
    desc.constraints.required = false;  // Optional
    desc.constraints.default_value = std::string("");
  ...
  // efm_path parameter
  {
    ...
    desc.description =
        "Path to the EFM data .efm file (8-bit t-values from 3-11)";
    desc.type = ParameterType::FILE_PATH;
    desc.constraints.required = false;  // Optional
    desc.constraints.default_value = std::string("");
  ...
  // ac3rf_path parameter
  {
    ...
    desc.description =
        "Path to the AC3 RF symbols .ac3sym file (demodulated QPSK dibits from "
        "ld-decode)";
    desc.type = ParameterType::FILE_PATH;
    desc.constraints.required = false;  // Optional
    desc.constraints.default_value = std::string("");
...
std::map<std::string, ParameterValue> NTSCYCSourceStage::get_parameters()
    const {
...
bool NTSCYCSourceStage::set_parameters(
...
std::optional<StageReport> NTSCYCSourceStage::generate_report() const {
  ...
  // Get db_path
  std::string effective_db_path =
      db_path_.empty() ? (y_path_ + ".db") : db_path_;
  ...
  try {
    auto representation = load_representation(
        y_path_, c_path_, effective_db_path, pcm_path_, efm_path_, ac3rf_path_);
    if (representation) {
      ...
      if (video_params) {
        ...
        report.items.push_back(
            {"Field Dimensions",
             std::to_string(video_params->frame_width_nominal) + " x " +
                 std::to_string(
                     calculate_padded_field_height(video_params->system))});
        const int64_t total_fields =
            static_cast<int64_t>(video_params->number_of_sequential_frames) * 2;
        report.items.push_back(
            {"Total Fields", std::to_string(total_fields)});
        report.items.push_back(
            {"Total Frames",
             std::to_string(video_params->number_of_sequential_frames)});

        // Metrics
        report.metrics["field_count"] = total_fields;
        report.metrics["frame_count"] =
            static_cast<int64_t>(video_params->number_of_sequential_frames);
        report.metrics["field_width"] =
            static_cast<int64_t>(video_params->frame_width_nominal);
        report.metrics["field_height"] = static_cast<int64_t>(
            calculate_padded_field_height(video_params->system));
      }
    } else {
...
std::vector<PreviewOption> NTSCYCSourceStage::get_preview_options() const {
  ...
  // The GUI will detect has_separate_channels() and provide a separate Signal
  // dropdown (Y/C/Y+C). When calling render_preview(), the GUI combines mode +
  // channel (e.g., "field_y", "split_c").
...
PreviewImage NTSCYCSourceStage::render_preview(
  ...
  // Expected formats: field_y, field_y_raw, split_c, split_c_raw, frame_yc,
  // frame_yc_raw
