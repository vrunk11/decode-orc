...
bool AudioSinkStage::trigger(
    ...
    try {
        ...
        ORC_LOG_INFO("AudioSink: Successfully wrote {} frames to {}", write_result.frames_written, output_path);
        last_status_ = "Success: " + std::to_string(write_result.frames_written) + " samples written";
        is_processing_.store(false);
        return true;
