 ...
 * Purpose:     PAL TBC YC variant — Y/C phase alignment check
 *
...
// PalTBCYCConverter
// ---------------------------------------------------------------------------
// Phase alignment helpers for PAL TBC sources that carry separate luma (Y)
...
//
// EBU Tech. 3280-E §1.1.1: PAL colour_frame_index is 1–4.  A mismatch means
// the Y and C files are from different points in the 4-frame sequence.
class PalTBCYCConverter {
 public:
  ...
  static std::string yc_alignment_error(int luma_colour_frame_index,
                                        int chroma_colour_frame_index) {
    return orc::yc_alignment_error("PAL", luma_colour_frame_index,
                                   chroma_colour_frame_index);
  }
};
