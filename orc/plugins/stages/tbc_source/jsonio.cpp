...
#include <limits>
    ...
    (defined(__GNUC__) && __GNUC__ >= 11 && __cplusplus >= 201703L)
#define USE_CHARCONV
#endif
#ifdef USE_CHARCONV
#include <charconv>
...
void JsonReader::read(int& value) { readSignedInteger(value); }

void JsonReader::read(int64_t& value) { readSignedInteger(value); }

void JsonReader::read(double& value) { readNumber(value); }
...
void JsonReader::read(std::string& value) { readString(value); }
...
void JsonReader::readNumber(double& value) {
  ...
  // (Skip if clang is detected since it was late to implement, and thus not
...
JsonWriter::JsonWriter(std::ostream& _output) : output(_output), atStart(true) {
  // Use maximum double precision for floating-point output
  output.precision(std::numeric_limits<double>::digits10 + 1);
}

void JsonWriter::write(int value) { output << value; }

void JsonWriter::write(int64_t value) { output << value; }

void JsonWriter::write(double value) { output << value; }

void JsonWriter::write(bool value) { output << (value ? "true" : "false"); }

void JsonWriter::write(const char* value) { writeString(value); }

void JsonWriter::write(const std::string& value) { writeString(value.c_str()); }
