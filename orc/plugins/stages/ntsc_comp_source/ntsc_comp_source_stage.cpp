...
NTSCCompSourceStage::NTSCCompSourceStage(
    std::shared_ptr<INTSCCompSourceLoader> loader)
...
NTSCCompSourceStage::load_representation(const std::string& input_path,
...
std::vector<ArtifactPtr> NTSCCompSourceStage::execute(
  ...
  if (input_path_it == parameters.end() ||
    ...
    // placeholder
    ORC_LOG_DEBUG(
        "NTSC_Comp_Source: No input_path configured, returning empty output");
    return {};
  }
  ...
  if (cached_representation_ && cached_input_path_ == input_path) {
    ORC_LOG_DEBUG("NTSC_Comp_Source: Using cached representation for {}",
                  input_path);
  ...
  try {
    ...
    if (video_params->system != VideoSystem::NTSC) {
      throw UserDataError(
          "TBC file is not NTSC format. Use 'Add PAL Composite Source' for PAL "
          "files.");
    }
    ...
    throw UserDataError(std::string("Failed to load NTSC TBC file '") +
                        input_path + "': " + e.what());
  }
}

std::vector<ParameterDescriptor> NTSCCompSourceStage::get_parameter_descriptors(
  ...
  // input_path parameter
  {
    ...
    desc.description =
        "Path to the NTSC .tbc file from ld-decode (database file is "
...
std::map<std::string, ParameterValue> NTSCCompSourceStage::get_parameters()
    const {
  return parameters_;
}

bool NTSCCompSourceStage::set_parameters(
...
PreviewImage NTSCCompSourceStage::render_preview(
