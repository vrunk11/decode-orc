...
inline LogCategoryFlags operator|(LogCategoryFlags a, LogCategoryFlags b) {
  return static_cast<LogCategoryFlags>(static_cast<int>(a) |
                                       static_cast<int>(b));
}
...
class Logger {
 public:
  ...
  virtual void log(LogPriority priority, LogCategoryFlags categorization,
                   const std::string& message) = 0;
  [[nodiscard]] virtual bool isEnabled(
      LogPriority priority, LogCategoryFlags categorization) const = 0;
  virtual void sync() = 0;
