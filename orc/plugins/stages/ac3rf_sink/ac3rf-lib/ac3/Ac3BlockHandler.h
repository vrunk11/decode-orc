...
class Ac3BlockHandler {
 public:
  ...
  // Keeps track of received frames and returns a block of 72 frames when they
  // have been collected
  std::optional<UncorrectedBlock> handleFrame(
      const Ac3InputFraming::NumberedFrame& frame);

  // Runs Reed Solomon error correction on a single block, including
  // de-interleaving This method has no state, but cannot be static since it
  // uses the RS decoders and also the logger.
  std::optional<CorrectedBlock> errorCorrectBlock(
      const UncorrectedBlock& block);

  // Parses corrected blocks to find the AC3 preample and returns data bursts.
  std::vector<std::array<uint8_t, 1536>> handleCorrectedBlock(
