...
std::optional<Ac3BlockHandler::UncorrectedBlock> Ac3BlockHandler::handleFrame(
    const Ac3InputFraming::NumberedFrame& frame) {
  int usedFrameNo;
  ...
  if (usedFrameNo == 71) {  // block completed
    m_expected_seq = 0;
    m_current_block.global_symbol_index =
        frame.global_symbol_index - static_cast<size_t>(71 * 37 * 4);
...
std::optional<Ac3BlockHandler::CorrectedBlock>
Ac3BlockHandler::errorCorrectBlock(const UncorrectedBlock& block) {
  ...
  for (int k = 0; k < 36; k++) {
    for (int i = 0; i < 37; i++) {
      ...
      int i2 = i * 2 + 1;
      c1Data1[i] =
          ByteWithErasureFlag(block.block_data[k * 2 + i1 / 37][i1 % 37]);
      c1Data2[i] =
          ByteWithErasureFlag(block.block_data[k * 2 + i2 / 37][i2 % 37]);
    }
    m_c1.decode(c1Data1);
    m_c1.decode(c1Data2);
    for (int i = 0; i < 33; i++) {
  ...
  // c2CorrectedBlock has the column codewords from c1CorrectedBlock in its rows
  // (with parity bytes dropped) Maybe we could do something if detecting
  // erasures -- like marking the block as bad?
  CorrectedBlock c2CorrectedBlock;

  std::vector<ByteWithErasureFlag> c2Data;
  c2Data.resize(36);
  for (int k = 0; k < 66; k++) {
    for (int i = 0; i < 36; i++) c2Data[i] = c1CorrectedBlock[k + i * 66];
    m_c2.decode(c2Data);
    for (int i = 0; i < 32; i++) {
  ...
  if (c2CorrectedBlock.block_data[0][0] != 0x10 ||
      c2CorrectedBlock.block_data[0][1] != 0x00) {
...
std::vector<std::array<uint8_t, 1536>> Ac3BlockHandler::handleCorrectedBlock(
  ...
  for (int i = 0; i < 66; i++) {
    ...
    for (int j = i == 0 ? 2 : 0; j < 32; j++) {
      ...
      if (m_ac3_output_block_index == 0) {
        ...
        // input blocks that are on average starting every 11520 input symbol
        ...
        if (m_last_burst_symbol_index != -1 &&
          ...
          m_log.warn(
              eAudio,
              fmt::format(
                  "Some input was not recognized as AC3 data and was "
                  "discarded: {} symbols / {} bytes / {} bursts",
                  m_first_symbol_in_burst - m_last_burst_symbol_index,
                  (m_first_symbol_in_burst - m_last_burst_symbol_index) / 4,
                  (m_first_symbol_in_burst - m_last_burst_symbol_index) /
                      (static_cast<size_t>(2880 * 4))));
        }
...
std::string Ac3BlockHandler::reedSolomonStatistics() {
  return "C1 statistics: " + m_c1.statistics() +
         "C2 statistics: " + m_c2.statistics();
}
