...
Ac3Decoder::Ac3Decoder(Logger& log)
    : m_input_framer(log), m_block_handler(log) {}

std::vector<std::array<uint8_t, 1536>> Ac3Decoder::decodeSymbols(
  ...
  for (auto frame : frames) {
    if (auto block = m_block_handler.handleFrame(frame); block.has_value()) {
      if (auto correctedBlock =
              m_block_handler.errorCorrectBlock(block.value());
          correctedBlock.has_value()) {
        auto output =
