...
Ac3InputFraming::Ac3InputFraming(Logger& log) : m_log(log) {}

// Finds the sync pattens and breaks the sequence up into frames
std::vector<Ac3InputFraming::NumberedFrame> Ac3InputFraming::arrangeInFrames(
  ...
  for (int i = 0; i < symbols.size(); i++, m_total_symbols_seen++) {
    ...
    if (m_sync_frame_symbols_seen < 12 &&
        m_auto_sync_at < m_total_symbols_seen) {
      ...
      if (is_next_sync_symbol) {
        ...
        if (m_consecutive_synched > 0) {
          m_log.debug(
              eAudio,
              fmt::format(
                  "Missing sync symbol index {} (consecutiveSynched={})!",
                  m_total_symbols_seen, m_consecutive_synched));
          m_consecutive_synched -= 1;
          m_frame_number = (m_previous_frame_number + 1) % 72;
          m_auto_sync_at =
              m_total_symbols_seen + 12 - m_sync_frame_symbols_seen;
        } else {
      ...
      if (m_symbol_in_frame_counter == 37 * 4) {
        ...
        for (int j = 0; j < 37; j++) {
          frame[j] = m_symbols_in_frame[static_cast<ptrdiff_t>(j * 4)] << 6 |
