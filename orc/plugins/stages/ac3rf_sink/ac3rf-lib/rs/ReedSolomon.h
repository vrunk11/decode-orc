...
// This file is licensed under the provisions of the Gnu General Public License
// v3 (see gpl-3.0.txt)
...
enum DecodingStrategy {
  ...
  RS_C1,    // Very conservative: only corrects one error and erases the entire
            // frame otherwise
  RS_C2,    // Assumes few unflagged errors
  RS_MAX    // Tries its best to correct as many erasures/errors possible (with
            // the risk of incorrect corrections)
};

template <int irreducible_poly, int alpha_decimal>
class ReedSolomon {
 public:
  typedef GFValue<8, irreducible_poly, alpha_decimal> GF;

  // Notice this is only tested for k = n - 4.
  ReedSolomon(int n, int k, int fcr, DecodingStrategy decoding_strategy,
              bool create_diagnotics)
 ...
 private:
  ...
  void addDiagnosticCounter(std::string const& name) {
    if (m_create_diagnotics) m_diagnostics[name]++;
  }
  ...
  std::vector<GF> computeSyndromes(
      std::vector<ByteWithErasureFlag> const& data) {
  ...
  void eraseAll(std::vector<ByteWithErasureFlag>& data) {
    for (auto& b : data) b.setErased(true);
  }

  void uneraseAll(std::vector<ByteWithErasureFlag>& data) {
    for (auto& b : data) b.setErased(false);
    addDiagnosticCounter("unerased all");
  }

  bool tryCorrectOneError(std::vector<GF> const& syndromes,
                          std::vector<ByteWithErasureFlag>& data) {
    if (std::count_if(syndromes.cbegin(), syndromes.cend(),
                      [](GF a) -> bool { return a.isZero(); })) {
      ...
      if (errorPos0 < 0 || errorPos0 >= m_n) {
        ...
        data[errorPos0] = ByteWithErasureFlag(
            data[errorPos0].gfValue<irreducible_poly, alpha_decimal>() + y0);
  ...
  bool tryCorrectTwoErrors(std::vector<GF> const& syndromes,
                           std::vector<ByteWithErasureFlag>& data) {
    auto determinant =
        syndromes[1] * syndromes[1] + syndromes[0] * syndromes[2];
    if (determinant.isZero()) {
      ...
      auto lambda1 =
          (syndromes[1] * syndromes[2] + syndromes[0] * syndromes[3]) * detInv;
      auto lambda2 =
          (syndromes[2] * syndromes[2] + syndromes[1] * syndromes[3]) * detInv;

      std::vector<std::pair<GF, int>> roots;
      auto alphaPowI = GF(1);
      auto lambda1TimesAlphaPowI = lambda1;  // for i == 0
      auto lambda2TimesAlphaPow2I =
          GF(1);  // we swap lambda0 (which is 1) and lambda2 in order to get
                  // the error locations directly without inversion
      for (int i = 0; i < m_n; i++) {
        ...
        //        println(f"$i: ${alphaPowI.a}%02x ${alphaPowMinusI.a}%02x
        //        ${lambda1TimesAlphaPowI.a}%02x
        //        ${lambda2TimesAlphaPow2I.a}%02x")
      }
      ...
      if (roots.size() != 2) {
        ...
        assert(errorPos0 >= 0 && errorPos0 < m_n && errorPos1 >= 0 &&
               errorPos1 < m_n);

        auto x0PlusX1Inv = (x0 + x1).inverse();
        auto y0 = (syndromes[1] + syndromes[0] * x1) * x0PlusX1Inv *
                  m_alpha.pow(errorPos0 * m_fcr).inverse();
        auto y1 = (syndromes[1] + syndromes[0] * x0) * x0PlusX1Inv *
                  m_alpha.pow(errorPos1 * m_fcr).inverse();

        ByteWithErasureFlag data0 = data[errorPos0];
        ByteWithErasureFlag data1 = data[errorPos1];
        data[errorPos0] = ByteWithErasureFlag(
            data0.gfValue<irreducible_poly, alpha_decimal>() + y0);
        data[errorPos1] = ByteWithErasureFlag(
            data1.gfValue<irreducible_poly, alpha_decimal>() + y1);
  ...
  bool tryDecodeErasures(std::vector<GF> const& syndromes,
                         std::vector<ByteWithErasureFlag>& data) {
    std::vector<int> erasure_positions;
    for (int i = 0; i < m_n; i++)
      if (data[i].isErased()) erasure_positions.push_back(i);
    if (erasure_positions.size() > syndromes.size()) {
      addDiagnosticCounter(
          fmt::format("erasure decoding not performed for {} erasures",
                      erasure_positions.size()));
      return false;
    } else {
      std::vector<GF> x(erasure_positions.size());
      std::transform(erasure_positions.begin(), erasure_positions.end(),
                     x.begin(), [this](int b) -> GF { return m_alpha.pow(b); });
      auto s = syndromes;
      ...
      for (int i = 0; i < erasure_positions.size(); i++) {
        ...
        auto error = y[i];
        data[errorPos] = ByteWithErasureFlag(
            data[errorPos].gfValue<irreducible_poly, alpha_decimal>() +
            error * m_alpha.pow(errorPos * m_fcr).inverse());
      }

      auto new_syndromes = computeSyndromes(data);
      if (std::count_if(new_syndromes.cbegin(), new_syndromes.cend(),
                        [](GF a) -> bool { return a.nonZero(); }) != 0) {
        ...
        addDiagnosticCounter(
            fmt::format("erasure decoding failed for {} erasures",
                        erasure_positions.size()));
        return false;
      } else {
        addDiagnosticCounter(
            fmt::format("corrected {} erasures", erasure_positions.size()));
        return true;
      }
    }
  }

  bool tryDecodeOneErrorTwoErasures(std::vector<GF> const& syndromes,
                                    std::vector<ByteWithErasureFlag>& data) {
    std::vector<int> erasure_positions;
    for (int i = 0; i < m_n; i++)
      if (data[i].isErased()) erasure_positions.push_back(i);
    if (erasure_positions.size() != 2) {
      ...
      // See "Method for correcting both errors and erasures of RS codes using
      // error-only and erasure-only decoding algorithms" by Erl-Huei Lu,
      // Pen-Yao Lu, Tso-Cho Chen (https://doi.org/10.1049/el.2013.1521)

      // Think of y0 and y1 being the erased errors at locators x0 and x1
      auto alphaInv = m_alpha_inverse;
      auto s0prime =
          syndromes[0] + syndromes[1] * alphaInv.pow(erasure_positions[0]);
      ...
      auto s1bis =
          s1prime * alphaInv.pow(posDiff) + s2prime * alphaInv.pow(2 * posDiff);

      if (s0bis == GF(0) || s1bis == GF(0)) {
        addDiagnosticCounter(
            "single correction with two erasures failed s0bis or s1bis zero");
        return false;
      } else {
        ...
        if (errorPos2 < 0 || errorPos2 >= m_n) {
          addDiagnosticCounter(
              "single correction with two erasures failed errorPos2 out of "
              "range");
          return false;
        } else {
          auto y2prime = s0bis;
          ...
          if (check1 == GF(0) || check2 == GF(0)) {
            addDiagnosticCounter(fmt::format(
                "single correction with two erasures failed zero inverse {} {}",
                check1.getInt(), check2.getInt()));
            return false;
          } else {
            auto y2BeforeFcrAdjust = y2prime *
                                     (GF(1) + x0.inverse() * x2).inverse() *
                                     (GF(1) + x2prime).inverse();
            auto y2 =
                y2BeforeFcrAdjust * GF::alpha_pow(errorPos2 * m_fcr).inverse();

            data[errorPos2] = ByteWithErasureFlag(
                data[errorPos2].gfValue<irreducible_poly, alpha_decimal>() +
                y2);

            auto fixedSyndromes = std::vector<GF>{
                syndromes[0] + y2BeforeFcrAdjust,
                syndromes[1] + y2BeforeFcrAdjust * GF::alpha_pow(errorPos2)};
            ...
            if (c) {
              ...
              data[errorPos2] = ByteWithErasureFlag(
                  ...
                  y2);
              addDiagnosticCounter(
                  "single correction with two erasures failed");
            }
            return c;
          }
        }
   ...
   */
  std::vector<GF> solveSyndromeEquations(std::vector<GF> const& x,
                                         std::vector<GF> const& s) {
    int m = (int)x.size();
    assert(s.size() == m);
    assert(std::count_if(x.cbegin(), x.cend(),
                         [](GF a) -> bool { return a.isZero(); }) == 0);
    ...
    for (int i = 0; i < m - 1; i++) {  // for each row, clear column i under it
      ...
      for (int k = i + 1; k < m; k++) {  // rows under
        ...
        for (int j = i; j < m; j++) mat[k][j] = mat[k][j] + leading * mat[i][j];
        y[k] = y[k] + leading * y[i];
      }
    }
...
void ReedSolomon<irreducible_poly, alpha>::decode(
    std::vector<ByteWithErasureFlag>& data) {
  assert(data.size() == m_n);

  // we want the right end of the codeword to have location 0 -- this makes the
  // index for coefficients the same as their corresponding monomial power
...
template <int irreducible_poly, int alpha>
void ReedSolomon<irreducible_poly, alpha>::doDecode(
    std::vector<ByteWithErasureFlag>& data) {
  assert(data.size() == m_n);
  int number_of_erasures =
      std::count_if(data.cbegin(), data.cend(),
                    [](ByteWithErasureFlag b) -> bool { return b.isErased(); });
  auto syndromes = computeSyndromes(data);

  auto data_copy = data;  // save to print original if we think we decoded
                          // correctly but didn't

  if (std::all_of(syndromes.cbegin(), syndromes.cend(),
                  [](GFValue<8, irreducible_poly, alpha> s) -> bool {
                    return s.isZero();
                  })) {
    if (number_of_erasures == 0) {
      ...
      uneraseAll(data);
      addDiagnosticCounter(fmt::format(
          "syndromes zero with {} erasures, un-erased", number_of_erasures));
      ...
      case RS_MAX: {
        ...
        if (false && number_of_erasures >= 1) {
          if (tryDecodeErasures(syndromes, data)) {
            ...
            if (number_of_erasures == 2)  // since erasure decoding failed we
                                          // know we have at least one error
              corrected = tryDecodeOneErrorTwoErasures(syndromes, data);
            else if (number_of_erasures == 1 && determinant != GF(0))
              corrected = tryCorrectTwoErrors(syndromes, data);
            else if (number_of_erasures == 1 &&
                     determinant == GF(0))  // in this case the erasure is wrong
                                            // -- risk of incorrect decoding?
        ...
        if (corrected) {
          ...
          if (number_of_non_zero != 0) {
            ...
            for (int i = 0; i < data_copy.size(); i++) {
              printf("{0x%x, %s}, ", data_copy[i].byteValue(),
                     data_copy[i].isErased() ? "true" : "false");
              if (i % 4 == 3) printf("\n");
            }
            printf("\n");
          }
          if (number_of_erasures != 0) uneraseAll(data);
        } else
          eraseAll(data);
      }
    }
