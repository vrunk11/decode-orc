...
ORC_STAGE_PLUGIN_EXPORT bool orc_register_stage_plugin(
  ...
  if (node_type_info.display_name != orc::plugins::ld_sink::kStageDisplayName ||
      node_type_info.menu_category !=
          orc::plugins::ld_sink::kStageMenuCategory ||
      ...
      node_type_info.compatible_formats !=
          orc::plugins::ld_sink::kStageCompatibleFormats ||
      node_type_info.sink_category !=
          orc::plugins::ld_sink::kStageSinkCategory) {
    if (error_message) {
      *error_message =
  ...
  if (!register_stage(context, orc::plugins::ld_sink::kStageName,
                      &create_stage)) {
    if (error_message) {
