...
StagePluginRegistryEntry parse_plugin_entry(const YAML::Node& node, std::vector<std::string>& warnings)
{
    ...
    if (entry.artifact_source != "local_path" && entry.artifact_source != "github_release_asset") {
        warnings.push_back(
            "Registry entry has unknown artifact_source '" + entry.artifact_source +
            "'; expected 'local_path' or 'github_release_asset'");
    }

    if (!entry.release_asset_name.empty() && !is_valid_release_asset_name(entry.release_asset_name)) {
        warnings.push_back(
            "Registry entry has invalid release_asset_name '" + entry.release_asset_name +
            "' (expected orc-plugin_<stage-name>_<platform>.<so|dylib|dll>)");
    }

    if (const auto repo_name = extract_github_repo_name(entry.source_repo_url); repo_name.has_value()) {
        if (repo_name->rfind("orc-plugin_", 0) != 0) {
            warnings.push_back(
                "Registry entry source_repo_url should reference a repository prefixed with 'orc-plugin_'"
                " (found '" + *repo_name + "')");
        }
    }
    ...
    if (entry.path.empty()) {
        if (entry.artifact_source == "github_release_asset" && !entry.release_asset_url.empty()) {
            ...
            if (entry.release_asset_name.empty()) {
                warnings.push_back(
                    "Registry entry references a remote release asset but has no release_asset_name "
    ...
    const std::string& registry_path)
{
    ...
    try {
        ...
        result.warnings.push_back(std::string("Failed to parse plugin registry YAML: ") + e.what());
        return result;
    }
    ...
    if (version > kRegistrySchemaVersion) {
        result.warnings.push_back(
            "Unexpected plugin registry schema version " + std::to_string(version) +
            "; expected " + std::to_string(kRegistrySchemaVersion));
    }
    ...
    for (const auto& plugin_node : plugins_node) {
        ...
        // If entry has no path but references a remote release asset, attempt to download it
        if (entry.path.empty() && entry.artifact_source == "github_release_asset" &&
            ...
            auto download_result =
                PluginRemoteLoader::download_release_asset(entry.release_asset_url,
            ...
            if (download_result.success) {
                ...
                result.warnings.push_back("Failed to download plugin '" + entry.plugin_id +
                                         "': " + download_result.error_message);
                continue;  // Skip this entry if download failed
            }
        }

        // Only add entries that have a valid path (either pre-existing or downloaded)
        if (entry.path.empty()) {
            continue;
        }
...
StagePluginRegistry::LoadResult StagePluginRegistry::load_default()
{
    ...
    if (!std::filesystem::exists(path)) {
        result.warnings.push_back("Plugin registry file not found; starting with empty registry");
        return result;
    }

    std::ifstream file(result.registry_path, std::ios::in);
    if (!file) {
        result.warnings.push_back("Failed to open plugin registry file for reading");
        return result;
    }
