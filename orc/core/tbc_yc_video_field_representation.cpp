...
void TBCYCVideoFieldRepresentation::ensure_video_parameters() {
  if (metadata_reader_ && metadata_reader_->is_open()) {
    ...
    if (params_opt) {
      ...
      ORC_LOG_DEBUG(
          "TBCYCVideoFieldRepresentation: FSC for system = {} Hz",
          fsc_from_system(video_params_.system));
    }
  }
}
...
std::optional<FieldDescriptor> TBCYCVideoFieldRepresentation::get_descriptor(
    FieldID id) const {
  if (!has_field(id)) {
    return std::nullopt;
  }
  ...
  if (video_params_.is_valid()) {
    ...
    desc.width = static_cast<size_t>(video_params_.frame_width_nominal);
    ...
    if (parity_hint.has_value()) {
      ...
      ORC_LOG_WARN(
          "TBCYCVideoFieldRepresentation: No parity hint for field {}, using "
          "ID-based inference",
          id.value());
    }
...
TBCYCVideoFieldRepresentation::get_field(FieldID /*id*/) const {
  // YC sources don't provide composite (Y+C modulated) data
  // Downstream stages should use get_field_luma() and get_field_chroma()
  // instead
  return {};
}
...
TBCYCVideoFieldRepresentation::get_line_luma(FieldID id, size_t line) const {
  ...
  if (cached_field) {
    ...
    size_t width = static_cast<size_t>(video_params_.frame_width_nominal);
...
TBCYCVideoFieldRepresentation::get_field_luma(FieldID id) const {
  ...
  ORC_LOG_DEBUG(
      "TBCYCVideoFieldRepresentation: Reading luma field {} from Y reader",
      id.value());
  ...
  size_t line_length = static_cast<size_t>(video_params_.frame_width_nominal);
  ...
  if (field_data.size() > actual_samples) {
    ...
    ORC_LOG_DEBUG(
        "TBCYCVideoFieldRepresentation: Truncated luma field {} from {} to {} "
  ...
  ORC_LOG_DEBUG("Luma field {} first samples: {} {} {} {} {}", id.value(),
...
TBCYCVideoFieldRepresentation::get_field_chroma(FieldID id) const {
  ...
  ORC_LOG_DEBUG(
      "TBCYCVideoFieldRepresentation: Reading chroma field {} from C reader",
      id.value());
  ...
  if (field_data.size() > actual_samples) {
    field_data.resize(actual_samples);
    ORC_LOG_DEBUG(
        "TBCYCVideoFieldRepresentation: Truncated chroma field {} from {} to "
        "{} samples (removed padding)",
        id.value(),
        field_data.size() + (field_data.capacity() - field_data.size()),
        actual_samples);
  ...
  // Debug: Log first few samples
  ORC_LOG_DEBUG("Chroma field {} first samples: {} {} {} {} {}", id.value(),
                field_data[0], field_data[1], field_data[2], field_data[3],
                field_data[4]);
...
std::optional<FieldMetadata> TBCYCVideoFieldRepresentation::get_field_metadata(
    FieldID id) const {
  if (!metadata_reader_ || !metadata_reader_->is_open()) {
    return std::nullopt;
  }
...
std::shared_ptr<TBCYCVideoFieldRepresentation> create_tbc_yc_representation(
    ...
    const std::string& efm_filename, const std::string& ac3rf_filename) {
  // Create readers
  ...
  const size_t frame_width = static_cast<size_t>(params.frame_width_nominal);
  size_t field_length = frame_width * padded_field_height;

  // Open Y (luma) file
  ORC_LOG_DEBUG("Opening Y (luma) file: {}", y_filename);
  if (!y_reader->open(y_filename, field_length, frame_width)) {
    ORC_LOG_ERROR("Failed to open Y (luma) TBC file: {}", y_filename);
    return nullptr;
  }
  ORC_LOG_DEBUG("Y file opened successfully, {} fields detected",
                y_reader->get_field_count());

  // Open C (chroma) file
  ORC_LOG_DEBUG("Opening C (chroma) file: {}", c_filename);
  if (!c_reader->open(c_filename, field_length, frame_width)) {
    ORC_LOG_ERROR("Failed to open C (chroma) TBC file: {}", c_filename);
    return nullptr;
  }
  ORC_LOG_DEBUG("C file opened successfully, {} fields detected",
                c_reader->get_field_count());
  ...
  size_t metadata_field_count =
      static_cast<size_t>(params.number_of_sequential_frames) * 2;
  ...
  if (y_field_count != metadata_field_count) {
    ...
    ORC_LOG_ERROR("  Y file: {} contains {} fields ({} bytes)", y_filename,
                  y_field_count, actual_y_file_size);
    ORC_LOG_ERROR("  C file: {} contains {} fields ({} bytes)", c_filename,
                  c_field_count, actual_c_file_size);
    ...
    ORC_LOG_ERROR(
        "  These files may be corrupted or truncated. Please regenerate the YC "
        "files.");
    return nullptr;
  }

  ORC_LOG_DEBUG("YC validation passed: {} fields, {}x{} pixels",
                metadata_field_count, params.frame_width_nominal,
                calculate_padded_field_height(params.system));
  ...
  auto representation = std::make_shared<TBCYCVideoFieldRepresentation>(
      y_reader, c_reader, metadata_reader, artifact_id, provenance);
...
std::shared_ptr<VideoFieldRepresentation> create_tbc_yc_source_representation(
    ...
    const std::string& metadata_filename, const std::string& pcm_filename,
    const std::string& efm_filename, const std::string& ac3rf_filename) {
  return create_tbc_yc_representation(y_filename, c_filename, metadata_filename,
                                      pcm_filename, efm_filename,
                                      ac3rf_filename);
}

std::vector<DropoutRegion> TBCYCVideoFieldRepresentation::get_dropout_hints(
    FieldID id) const {
...
TBCYCVideoFieldRepresentation::get_field_parity_hint(FieldID id) const {
  if (!metadata_reader_ || !metadata_reader_->is_open()) {
    return std::nullopt;
  }
...
std::optional<FieldPhaseHint>
TBCYCVideoFieldRepresentation::get_field_phase_hint(FieldID id) const {
  if (!metadata_reader_ || !metadata_reader_->is_open()) {
    return std::nullopt;
  }
...
TBCYCVideoFieldRepresentation::get_active_line_hint() const {
...
uint32_t TBCYCVideoFieldRepresentation::get_audio_sample_count(
    FieldID id) const {
  return audio_efm_handler_->get_audio_sample_count(id);
}

std::vector<int16_t> TBCYCVideoFieldRepresentation::get_audio_samples(
    FieldID id) const {
...
bool TBCYCVideoFieldRepresentation::set_audio_file(
...
std::vector<uint8_t> TBCYCVideoFieldRepresentation::get_efm_samples(
    FieldID id) const {
...
std::vector<uint8_t> TBCYCVideoFieldRepresentation::get_ac3_symbols(
    FieldID id) const {
...
bool TBCYCVideoFieldRepresentation::set_ac3rf_symbols_file(
