...
std::vector<std::string> split_plugin_search_paths(const std::string& raw_paths)
{
    ...
    for (const auto& raw_path : search_paths) {
        ...
        if (error_code) {
            diagnostics.push_back({
                StagePluginDiagnosticSeverity::Warning,
                raw_path,
        ...
        for (const auto& entry : std::filesystem::directory_iterator(path, error_code)) {
            if (error_code) {
                diagnostics.push_back({
                    StagePluginDiagnosticSeverity::Warning,
                    raw_path,
                    "Failed while enumerating plugin directory: " + error_code.message()});
                break;
            }
...
std::vector<std::string> collect_default_plugin_search_paths()
{
...
#if defined(_WIN32)
        const auto plugin_dir = (executable_dir / "orc-stage-plugins").lexically_normal().string();
#elif defined(__APPLE__)
        const auto plugin_dir = (executable_dir / ".." / "PlugIns" / "orc-stage-plugins").lexically_normal().string();
#else
        const auto plugin_dir = (executable_dir / ".." / "lib" / "orc-stage-plugins").lexically_normal().string();
#endif

        std::error_code error_code;
        if (std::filesystem::exists(std::filesystem::path(plugin_dir), error_code) && !error_code) {
    ...
    for (const auto& path : preferred_paths) {
        std::error_code error_code;
        if (std::filesystem::exists(std::filesystem::path(path), error_code) && !error_code && unique_paths.insert(path).second) {
...
const std::vector<LoadedStagePlugin>& StageRegistry::get_loaded_plugins() const {
    return plugin_loader_.loaded_plugins();
}

const std::vector<StagePluginDiagnostic>& StageRegistry::get_plugin_diagnostics() const {
...
const std::vector<StagePluginRegistryEntry>& StageRegistry::get_plugin_registry_entries() const {
...
void StageRegistry::initialize_runtime_plugins()
{
    ...
    for (const auto& warning : registry_result.warnings) {
        add_plugin_diagnostic(StagePluginDiagnosticSeverity::Warning, plugin_registry_path_, warning);
    }
    ...
    if (!registry_result.loaded_from_disk) {
        add_plugin_diagnostic(
            ...
            "Plugin registry file not loaded from disk; using in-memory empty registry");
    }
    ...
    const auto registry_paths = collect_registry_plugin_paths(plugin_registry_entries_, plugin_diagnostics_);
    ...
    if (!env_paths.empty()) {
        add_plugin_diagnostic(
            StagePluginDiagnosticSeverity::Info,
            "",
            "Using runtime plugin paths from ORC_STAGE_PLUGIN_PATHS in addition to YAML registry entries");
        unique_search_paths.insert(env_paths.begin(), env_paths.end());
    }

    plugin_search_paths_.assign(unique_search_paths.begin(), unique_search_paths.end());
    ...
    if (plugin_search_paths_.empty()) {
        add_plugin_diagnostic(
            StagePluginDiagnosticSeverity::Info,
            plugin_registry_path_,
            "No runtime stage plugins configured (registry has no enabled entries and ORC_STAGE_PLUGIN_PATHS is empty)");
        return;
    }

    auto plugin_candidates = discover_plugin_candidates(plugin_search_paths_, plugin_diagnostics_);
    if (plugin_candidates.empty()) {
        add_plugin_diagnostic(
            StagePluginDiagnosticSeverity::Info,
            "",
            "No runtime stage plugin binaries were discovered in configured search paths");
        return;
    }
    ...
    for (const auto& plugin_path : plugin_candidates) {
        ...
        auto load_result = plugin_loader_.load_plugin(
            plugin_path,
            [this, &pending_factories, &plugin_path](const std::string& stage_name, StagePluginLoader::RegisterStageFactory factory) {
                if (factories_.find(stage_name) != factories_.end() ||
                ...
                bool metadata_ok = core_internal::plugin_safe_call([&]{
                    try {
                        ...
                        add_plugin_diagnostic(
                            StagePluginDiagnosticSeverity::Error,
                            plugin_path,
                            "Plugin stage '" + stage_name + "' factory threw while validating metadata: " + ex.what());
                        stage = nullptr;
                        return;
                    } catch (...) {
                        add_plugin_diagnostic(
                            StagePluginDiagnosticSeverity::Error,
                            plugin_path,
                            "Plugin stage '" + stage_name + "' factory threw while validating metadata");
                        stage = nullptr;
                        return;
                    }
                    if (stage) {
                ...
                if (!metadata_ok) {
                    add_plugin_diagnostic(
                        StagePluginDiagnosticSeverity::Error,
                        plugin_path,
                        "Plugin stage '" + stage_name + "' caused a fatal fault during metadata validation: " + fault_error);
                    return false;
                }
                ...
                if (!stage) {
                    ...
                    bool already_diagnosed = [&]{
                        for (const auto& d : plugin_diagnostics_) {
                            if (d.message.find(stage_name) != std::string::npos) return true;
                        }
                        return false;
                    }();
                    if (!already_diagnosed) {
                        add_plugin_diagnostic(
                            StagePluginDiagnosticSeverity::Error,
                            plugin_path,
                            "Plugin stage '" + stage_name + "' returned null from factory during metadata validation");
                    }
                    return false;
                }
                if (node_type_info->display_name.empty()) {
                    add_plugin_diagnostic(
                        StagePluginDiagnosticSeverity::Error,
                        plugin_path,
                        "Plugin stage '" + stage_name + "' is missing required display_name metadata");
                    return false;
                }
                ...
                if (node_type_info->menu_category.empty()) {
                    add_plugin_diagnostic(
                        StagePluginDiagnosticSeverity::Error,
                        plugin_path,
                        "Plugin stage '" + stage_name + "' is missing required menu_category metadata");
                    return false;
                }
        ...
        if (!load_result.success) {
            add_plugin_diagnostic(StagePluginDiagnosticSeverity::Error, plugin_path, load_result.error_message);
            ORC_LOG_WARN("{}", load_result.error_message);
            continue;
        }

        const auto& plugin = *load_result.plugin;
        const std::string plugin_version = plugin.plugin_version.empty() ? std::string("unknown") : plugin.plugin_version;
        factories_.insert(pending_factories.begin(), pending_factories.end());
        add_plugin_diagnostic(
            StagePluginDiagnosticSeverity::Info,
            plugin_path,
            "Loaded plugin '" + plugin.plugin_id + "' v" + plugin_version + " registering " +
...
void StageRegistry::add_plugin_diagnostic(
    StagePluginDiagnosticSeverity severity,
