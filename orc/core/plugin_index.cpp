...
std::string serialize_plugin_index_yaml(const PluginIndex& index) {
  YAML::Node root;
  root["registry_schema"] = index.schema_version != 0
                                ? index.schema_version
                                : kPluginIndexSchemaVersion;
  ...
  for (const auto& entry : index.entries) {
    ...
    node["maintainer"] = entry.maintainer;
    ...
    for (const auto& artifact : entry.artifacts) {
      ...
      artifact_node["platform"] = artifact.platform;
      artifact_node["host_abi"] = artifact.host_abi;
      artifact_node["url"] = artifact.url;
      artifact_node["sha256"] = artifact.sha256;
      artifact_node["plugin_version"] = artifact.plugin_version;
      if (!artifact.min_host_app_version.empty()) {
        artifact_node["min_host_app_version"] = artifact.min_host_app_version;
      }
      if (!artifact.asset_name.empty()) {
        artifact_node["asset_name"] = artifact.asset_name;
      }
      artifacts.push_back(artifact_node);
    }
