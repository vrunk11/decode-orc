...
#error "GUI code cannot include core/include/plugin_remote_loader.h. Use ProjectPresenter for plugin-aware stage access."
#endif
#if defined(ORC_CLI_BUILD)
#error "CLI code cannot include core/include/plugin_remote_loader.h. Use ProjectPresenter for plugin-aware stage access."
#endif
...
public:
     ...
     *        Example: https://api.github.com/repos/owner/repo/releases/download/v1.0.0/plugin.so
     * @param asset_name Expected filename (must match orc-plugin_<stage>_<platform>.<ext> pattern)
     ...
     *
     * @return DownloadResult with success flag, local path (if success), and error message (if failure)
     *
     * On success, the binary is cached in ~/.config/decode-orc/plugin-cache/<platform>/
     * Subsequent calls with the same URL/asset_name retrieve the cached copy.
     */
    ...
    /**
     * Resolve a GitHub releases URL to a concrete release asset for the target platform.
     *
