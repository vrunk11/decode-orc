...
class TestPreviewStage final : public orc::DAGStage,
...
class TestColourPreviewStage final : public orc::DAGStage,
                                     public orc::IStagePreviewCapability,
 ...
 public:
  ...
  std::optional<orc::ColourFrameCarrier> get_colour_preview_carrier(
      uint64_t frame_index, orc::PreviewNavigationHint) const override {
...
class TestPreviewView final : public orc::IPreviewView {
 public:
  ...
  orc::PreviewViewDataResult request_data(
      orc::VideoDataType, const orc::PreviewCoordinate&) override {
    state_->request_called = true;

    if (state_->fail_request) {
      return {false, "request failed", orc::PreviewViewPayloadKind::None,
  ...
  orc::PreviewViewExportResult export_as(const std::string&,
...
// A stage that returns a ColourFrameCarrier without precomputed
// vectorscope_data. Used to verify that the registry can rebuild vectorscope
// samples on demand.
...
orc::DAG build_test_dag_with_stage(
    const std::shared_ptr<orc::DAGStage>& stage) {
  orc::DAG dag;
  orc::DAGNode node;
...
TEST(PreviewViewRegistryTest, Applicable_ViewsFilterByStageDataType) {
  ...
  const auto supported = registry.get_applicable_views(
      dag, orc::NodeID(1), orc::VideoDataType::CompositeNTSC);
  EXPECT_EQ(supported.size(), 1u);

  const auto unsupported = registry.get_applicable_views(
      dag, orc::NodeID(1), orc::VideoDataType::ColourPAL);
...
TEST(PreviewViewRegistryTest, Request_AndExportDispatchToViewInstance) {
  ...
  const auto request_result =
      registry.request_data(dag, orc::NodeID(1), "test.view",
                            orc::VideoDataType::CompositeNTSC, coordinate);

  EXPECT_TRUE(request_result.success);
  EXPECT_TRUE(state->request_called);

  const auto export_result =
      registry.export_as(orc::NodeID(1), "test.view", "png", "unused");
...
TEST(PreviewViewRegistryTest, RequestError_Propagates) {
  ...
  const auto request_result = registry.request_data(
      dag, orc::NodeID(1), "test.view", orc::VideoDataType::CompositeNTSC,
...
TEST(PreviewViewRegistryTest,
  ...
  EXPECT_EQ(vectorscope_result.payload_kind,
            orc::PreviewViewPayloadKind::Vectorscope);
  if (!vectorscope_result.vectorscope.has_value()) {
    FAIL() << "Expected vectorscope to have a value";
    return;
  }
...
TEST(PreviewViewRegistryTest, ReregisterAfterUnregister_Succeeds) {
  ...
  EXPECT_TRUE(registry.register_view({"test.view",
                                      "Test View Reregistered",
                                      {orc::VideoDataType::CompositeNTSC}},
                                     [state](orc::NodeID) {
                                       return std::make_unique<TestPreviewView>(
                                           state);
                                     }));
}
...
TEST(PreviewViewRegistryTest, RequestDataForUnknownViewId_ReturnsError) {
  ...
  const auto result = registry.request_data(
      dag, orc::NodeID(1), "unknown.view.id", orc::VideoDataType::CompositeNTSC,
      orc::PreviewCoordinate{});
...
TEST(PreviewViewRegistryTest,
  ...
  ASSERT_TRUE(registry.register_view(
      {"test.view",
       "Test View",
       {orc::VideoDataType::CompositeNTSC, orc::VideoDataType::ColourPAL}},
      [state](orc::NodeID) {
        return std::make_unique<TestPreviewView>(state);
      }));
  ...
  const auto result = registry.request_data(dag, orc::NodeID(1), "test.view",
...
TEST(PreviewViewRegistryTest,
  ...
  if (!result.vectorscope.has_value()) {
    FAIL() << "Expected vectorscope to have a value";
    return;
  }
