...
TEST_P(PublicStageContractTest, Parameter_DefaultsMatchRuntimeState) {
  ...
  for (const auto& descriptor : descriptors) {
    ...
    auto* parameterized_copy = dynamic_cast<orc::ParameterizedStage*>(
        stage_with_default_applied.get());
    ASSERT_NE(parameterized_copy, nullptr);
    ASSERT_TRUE(parameterized_copy->set_parameters(
        {{descriptor.name, *descriptor.constraints.default_value}}))
        << spec().inventory_id << " rejected descriptor default for '"
        << descriptor.name << "'";

    const auto applied_parameters = parameterized_copy->get_parameters();
    auto it = applied_parameters.find(descriptor.name);
    ASSERT_NE(it, applied_parameters.end())
        << spec().inventory_id << " did not expose applied default for '"
        << descriptor.name << "'";
...
TEST_P(PublicStageContractTest,
  ...
  for (const auto& key : required) {
    ...
    EXPECT_TRUE(required_keys.insert(key.full_key()).second)
        << spec().inventory_id << " duplicated required observation '"
        << key.full_key() << "'";
  }
  ...
  for (const auto& key : provided) {
    ...
    EXPECT_TRUE(provided_keys.insert(key.full_key()).second)
        << spec().inventory_id << " duplicated provided observation '"
        << key.full_key() << "'";
  }
  ...
  for (size_t index = 0; index < provided.size(); ++index) {
    ...
    EXPECT_NO_THROW(observation_context.set(
        orc::FieldID(static_cast<int32_t>(index)), key.namespace_, key.name,
        value_for_type(key.type)));

    if (key.type != orc::ObservationType::CUSTOM) {
      EXPECT_THROW(
          observation_context.set(orc::FieldID(static_cast<int32_t>(index)),
                                  key.namespace_, key.name,
                                  mismatched_value_for_type(key.type)),
...
TEST_P(PublicStageContractTest,
  ...
  if (completed_without_throw) {
    EXPECT_FALSE(returned)
        << spec().inventory_id
        << " unexpectedly accepted an invalid trigger invocation";
  }
}

INSTANTIATE_TEST_SUITE_P(PublicStages, PublicStageContractTest,
...
TEST_P(FormatSpecificDefaultsTest, PalFormat_DefaultsMatchRuntimeBehaviour) {
  ...
  for (const auto& desc : descriptors) {
    ...
    ASSERT_TRUE(
        fp->set_parameters({{desc.name, *desc.constraints.default_value}}))
        << spec().inventory_id << " rejected PAL descriptor default for '"
        << desc.name << "'";

    auto applied = fp->get_parameters();
    auto it = applied.find(desc.name);
    ASSERT_NE(it, applied.end())
        << spec().inventory_id << " did not expose applied PAL default for '"
        << desc.name << "'";
...
TEST_P(FormatSpecificDefaultsTest, NtscFormat_DefaultsMatchRuntimeBehaviour) {
  ...
  for (const auto& desc : descriptors) {
    ...
    ASSERT_TRUE(
        fp->set_parameters({{desc.name, *desc.constraints.default_value}}))
        << spec().inventory_id << " rejected NTSC descriptor default for '"
        << desc.name << "'";

    auto applied = fp->get_parameters();
    auto it = applied.find(desc.name);
    ASSERT_NE(it, applied.end())
        << spec().inventory_id << " did not expose applied NTSC default for '"
        << desc.name << "'";
    EXPECT_EQ(it->second, *desc.constraints.default_value)
...
INSTANTIATE_TEST_SUITE_P(PublicStages, FormatSpecificDefaultsTest,
                         testing::Range(size_t{0}, public_stage_specs().size()),
                         stage_param_name);
}  // namespace orc_unit_test