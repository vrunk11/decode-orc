 ...
 * Purpose:     Runtime smoke coverage for plugin loading through StagePluginLoader
 *
...
TEST(StagePluginLoaderSmokeTest, loadsRawVideoSinkPluginFromBuiltLibrary)
{
    ...
    EXPECT_EQ(loaded_stage_names.size(), result.plugin->registered_stage_names.size());
