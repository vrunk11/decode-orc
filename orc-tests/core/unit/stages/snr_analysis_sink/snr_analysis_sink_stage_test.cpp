 ...
 * Purpose:     Unit tests for SNRAnalysisSinkStage contracts and trigger
 * behavior
 *
 ...
 public:
  ...
  MOCK_METHOD(bool, write_csv,
              (const std::string& path,
               const std::vector<orc::FrameSNRStats>& frame_stats),
              (override));
};
...
TEST(SNRAnalysisSinkStageTest, Descriptor_DefaultsIncludeExpectedModes) {
  ...
  auto mode_it = std::find_if(
      ...
      [](const orc::ParameterDescriptor& d) { return d.name == "mode"; });

  ASSERT_NE(mode_it, descriptors.end());
  EXPECT_EQ(mode_it->type, orc::ParameterType::STRING);
  if (!mode_it->constraints.default_value.has_value()) {
    FAIL() << "Expected mode descriptor to have a default value";
    return;
  }
...
TEST(SNRAnalysisSinkStageTest, Trigger_UsesDepsSeamAndReportsSuccess) {
  ...
  EXPECT_CALL(*deps, compute_and_analyze(vfr.get(), _, _))
      .WillOnce(testing::DoAll(
          ...
          Return(orc::SNRAnalysisComputeResult{true, "SNR analysis complete",
                                               expected_stats, 240})));
...
TEST(SNRAnalysisSinkStageTest, Trigger_UsesDepsSeamAndPropagatesFailure) {
  ...
  EXPECT_CALL(*deps, compute_and_analyze(vfr.get(), _, _))
      .WillOnce(Return(
          orc::SNRAnalysisComputeResult{false, "observer failed", {}, 0}));
  EXPECT_CALL(*deps, write_csv(_, _)).Times(0);
...
TEST(SNRAnalysisSinkStageTest, Trigger_WritesCSVWhenDepsSucceeds) {
  ...
  EXPECT_CALL(*deps, compute_and_analyze(vfr.get(), _, _))
      .WillOnce(Return(orc::SNRAnalysisComputeResult{
          true, "SNR analysis complete", expected_stats, 8}));
