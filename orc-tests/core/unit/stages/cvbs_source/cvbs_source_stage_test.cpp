...
public:
    ...
    CVBSMetadataRecord metadata_record{"PAL", "CVBS_U16_4FSC", "STANDARD_TBC_LOCKED", "composite"};
    ...
    FakeCVBSSourceStageDeps()
    {
        ...
        payload_words.assign(static_cast<size_t>(910 * 525), static_cast<uint16_t>(240 * 64));
    }
                             ...
                             std::vector<uint16_t>& out_words, std::string& error_message) const override
    {
        ...
        out_words.assign(
            ...
            payload_words.begin() + static_cast<std::ptrdiff_t>(word_offset + word_count));
        return true;
    }
};
...
protected:
    ...
    std::shared_ptr<PALCVBSSourceStage> stage_ = std::make_shared<PALCVBSSourceStage>();
};
...
TEST_F(CVBSSourceContractTest, StageIdentity_ZeroRequiredInputs) {
...
TEST_F(CVBSSourceContractTest, Parameters_InputPathDescriptor) {
    ...
    auto input_path_it = std::find_if(descriptors.begin(), descriptors.end(),
...
TEST_F(CVBSSourceContractTest, Parameters_UseMetadataDescriptor) {
    ...
    auto use_metadata_it = std::find_if(descriptors.begin(), descriptors.end(),
...
TEST_F(CVBSSourceContractTest, Parameters_SampleEncodingDescriptor) {
    ...
    auto encoding_it = std::find_if(descriptors.begin(), descriptors.end(),
    ...
    EXPECT_FALSE(desc.constraints.required);
    ASSERT_TRUE(desc.constraints.default_value.has_value());
    EXPECT_EQ(std::get<std::string>(*desc.constraints.default_value), "CVBS_U16_4FSC");
    ...
    EXPECT_TRUE(std::find(desc.constraints.allowed_strings.begin(),
        desc.constraints.allowed_strings.end(), "CVBS_U16_4FSC") != desc.constraints.allowed_strings.end());
    EXPECT_TRUE(std::find(desc.constraints.allowed_strings.begin(),
        desc.constraints.allowed_strings.end(), "CVBS_TPG21_4FSC") != desc.constraints.allowed_strings.end());
}
...
TEST_F(CVBSSourceContractTest, ParameterSetGet_InputPath) {
    ...
    EXPECT_EQ(std::get<std::string>(retrieved["input_path"]), "/path/to/file.cvbs");
}

TEST_F(CVBSSourceContractTest, ParameterSetGet_UseMetadata) {
    std::map<std::string, ParameterValue> params;
...
TEST_F(CVBSSourceContractTest, ParameterSetGet_SampleEncoding) {
    std::map<std::string, ParameterValue> params;
    ...
    auto retrieved = stage_->get_parameters();
    EXPECT_EQ(std::get<std::string>(retrieved["sample_encoding"]), "CVBS_TPG21_4FSC");
}

TEST_F(CVBSSourceContractTest, ParameterSetGet_AllParameters) {
    std::map<std::string, ParameterValue> params;
    ...
    auto retrieved = stage_->get_parameters();
    EXPECT_EQ(std::get<std::string>(retrieved["input_path"]), "/path/to/cvbs.cvbs");
    EXPECT_TRUE(std::get<bool>(retrieved["use_metadata"]));
    EXPECT_EQ(std::get<std::string>(retrieved["sample_encoding"]), "CVBS_U16_4FSC");
}
...
TEST_F(CVBSSourceContractTest, ManualModeValidation_ValidPALStage_U16) {
    std::map<std::string, ParameterValue> params;
    ...
    EXPECT_TRUE(stage_->set_parameters(params));
    // Execution will throw UserDataError with clear message (Phase 2 not implemented)
}

TEST(CVBSSourceNTSCContractTest, ManualModeValidation_ValidNTSCStage_TPG21) {
...
TEST_F(CVBSSourceContractTest, ManualModeValidation_InvalidSampleEncoding) {
    std::map<std::string, ParameterValue> params;
...
TEST(CVBSSourceValidationTest, ManualMode_RejectsInvalidSampleEncodingAtExecute) {
...
TEST(CVBSSourceValidationTest, MetadataMode_ConstructsBasenameMetaPath) {
    ...
    std::vector<ArtifactPtr> inputs;
    ObservationContext observation_context;
    EXPECT_THROW((void)stage.execute(inputs, params, observation_context), UserDataError);
...
TEST(CVBSSourceValidationTest, MetadataMode_RejectsUnsupportedSampleEncoding) {
    ...
    expect_user_data_error_contains(stage, params, "Unsupported metadata sample_encoding_preset");
}
...
TEST(CVBSSourceValidationTest, MetadataMode_RejectsUnsupportedSignalState) {
    ...
    expect_user_data_error_contains(stage, params, "Unsupported metadata signal_state_preset");
}
...
TEST(CVBSSourceValidationTest, MetadataMode_RejectsNonCompositeSignalType) {
    ...
    expect_user_data_error_contains(stage, params, "Unsupported metadata signal_type");
}
...
TEST(CVBSSourceValidationTest, MetadataMode_SurfacesMetadataReadFailure) {
    ...
    expect_user_data_error_contains(stage, params, "metadata backend unavailable");
}
...
TEST(CVBSSourceValidationTest, MetadataMode_ValidMetadataProducesRepresentation) {
    auto deps = std::make_shared<FakeCVBSSourceStageDeps>();
    deps->metadata_record = CVBSMetadataRecord{"NTSC", "CVBS_TPG21_4FSC", "STANDARD_TBC_LOCKED", "composite"};
    const int16_t encoded = static_cast<int16_t>((300 - 508) * 64);
    deps->payload_words.assign(static_cast<size_t>(910 * 525), static_cast<uint16_t>(encoded));
    ...
    auto representation = std::dynamic_pointer_cast<VideoFieldRepresentation>(result.front());
...
TEST(CVBSSourceValidationTest, MetadataMode_AcceptsMatchingPalMetadata) {
    auto deps = std::make_shared<FakeCVBSSourceStageDeps>();
    deps->metadata_record = CVBSMetadataRecord{"PAL", "CVBS_U16_4FSC", "STANDARD_TBC_LOCKED", "composite"};
    deps->payload_words.assign(static_cast<size_t>(709379), static_cast<uint16_t>(256 * 64));
    PALCVBSSourceStage stage(deps);
...
TEST(CVBSSourceValidationTest, MetadataMode_RejectsMismatchedVideoStandard) {
    auto deps = std::make_shared<FakeCVBSSourceStageDeps>();
    deps->metadata_record = CVBSMetadataRecord{"NTSC", "CVBS_U16_4FSC", "STANDARD_TBC_LOCKED", "composite"};
    PALCVBSSourceStage stage(deps);
    ...
    expect_user_data_error_contains(stage, params, "Unsupported metadata preset 'NTSC'");
}

TEST(CVBSSourceDecodeTest, ManualMode_DecodesU16IntoCommonDomain) {
    auto deps = std::make_shared<FakeCVBSSourceStageDeps>();
    deps->payload_words.assign(static_cast<size_t>(910 * 525), static_cast<uint16_t>(252 * 64));
    NTSCCVBSSourceStage stage(deps);
...
TEST(CVBSSourceDecodeTest, ManualMode_ClampsLowTPG21SamplesIntoInternalDomain) {
    ...
    deps->payload_words.assign(static_cast<size_t>(910 * 525), static_cast<uint16_t>(encoded_below_zero));
    NTSCCVBSSourceStage stage(deps);
...
TEST(CVBSSourceDecodeTest, ManualMode_UsesPalFieldGeometry) {
    ...
    // Odd field: 311 lines @ 1135 samples + 2 fractional lines (156, 312) @ 1136 = 355,257
    // Even field: 310 lines @ 1135 samples + 2 fractional lines (155, 311) @ 1136 = 354,122
    deps->payload_words.assign(static_cast<size_t>(709379), static_cast<uint16_t>(256 * 64));
    PALCVBSSourceStage stage(deps);
...
TEST(CVBSSourceDecodeTest, ManualMode_DropsIncompleteFrameTail) {
    ...
    deps->payload_words.assign(ntsc_frame_samples + 100, static_cast<uint16_t>(240 * 64));
    NTSCCVBSSourceStage stage(deps);
...
TEST(CVBSSourceDecodeTest, ManualMode_NtscDescriptorParityMatchesTemporalOrder) {
    auto deps = std::make_shared<FakeCVBSSourceStageDeps>();
    deps->payload_words.assign(static_cast<size_t>(910 * 525), static_cast<uint16_t>(240 * 64));
    NTSCCVBSSourceStage stage(deps);
    ...
    ASSERT_TRUE(parity0.has_value());
    ASSERT_TRUE(parity1.has_value());
...
TEST(CVBSSourceDecodeTest, ManualMode_PreservesSyncCenteredLinesWithoutActiveWrap) {
    auto deps = std::make_shared<FakeCVBSSourceStageDeps>();
...
TEST(CVBSSourceDecodeTest, ManualMode_PalMultiFrameAlignmentConsistent) {
    auto deps = std::make_shared<FakeCVBSSourceStageDeps>();
    ...
    // Fill frames with sync and active patterns, accounting for variable-length lines
    ...
    for (FieldID::value_type field_idx = 0; field_idx < 6; ++field_idx) {
        ...
        ASSERT_NE(line0_ptr, nullptr) << "Field " << field_idx << " line 0 pointer is null";
        // Sync and active regions should still be recognizable, even if interpolation shifts them
...
TEST_F(CVBSSourceContractTest, Execute_SourceStageHasNoInputs) {
    std::vector<ArtifactPtr> inputs;
...
TEST_F(CVBSSourceContractTest, Execute_EmptyInputPathReturnsEmpty) {
    std::vector<ArtifactPtr> inputs;
    std::map<std::string, ParameterValue> parameters;
    parameters["input_path"] = std::string("");
...
TEST_F(CVBSSourceContractTest, Execute_InvalidPathSurfacesUserDataError) {
    std::vector<ArtifactPtr> inputs;
    std::map<std::string, ParameterValue> parameters;
    ...
    EXPECT_THROW(
        stage_->execute(inputs, parameters, observation_context),
        UserDataError
    );
}
...
TEST(CVBSSourcePreviewTest, PreviewAvailableAfterSuccessfulExecute) {
    ...
    const auto has_field_option = std::any_of(options.begin(), options.end(), [](const PreviewOption& option) {
        return option.id == "field";
    });
...
TEST(CVBSSourceReportTest, GenerateReportProvidesConfiguredFields) {
    ...
    const auto has_source_file = std::any_of(report->items.begin(), report->items.end(), [](const auto& item) {
        return item.first == "Source File" && item.second == "/virtual/input.cvbs";
    });
    EXPECT_TRUE(has_source_file);

    const auto has_mode = std::any_of(report->items.begin(), report->items.end(), [](const auto& item) {
