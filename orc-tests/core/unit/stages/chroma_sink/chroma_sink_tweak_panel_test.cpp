 ...
 * Purpose:     Unit tests for ChromaSinkStage live preview tweak parameter
 * exposure
 *
...
bool has_tweak(const orc::StagePreviewCapability& capability,
               const std::string& parameter_name,
               orc::PreviewTweakClass tweak_class) {
  return std::any_of(capability.tweakable_parameters.begin(),
                     capability.tweakable_parameters.end(),
                     [&](const orc::PreviewTweakableParameter& tweak) {
                       return tweak.parameter_name == parameter_name &&
                              tweak.tweak_class == tweak_class;
                     });
}
}  // namespace
...
TEST(ChromaSinkTweakPanelTest, Preview_CapabilityListsNtscDecodeTweaks) {
  ...
  EXPECT_TRUE(has_tweak(capability, "chroma_gain",
                        orc::PreviewTweakClass::DecodePhase));
  EXPECT_TRUE(has_tweak(capability, "chroma_phase",
                        orc::PreviewTweakClass::DecodePhase));
  EXPECT_TRUE(has_tweak(capability, "ntsc_phase_comp",
                        orc::PreviewTweakClass::DecodePhase));
  EXPECT_TRUE(has_tweak(capability, "chroma_weight",
                        orc::PreviewTweakClass::DecodePhase));
  EXPECT_TRUE(has_tweak(capability, "adapt_threshold",
                        orc::PreviewTweakClass::DecodePhase));

  EXPECT_FALSE(
      has_tweak(capability, "simple_pal", orc::PreviewTweakClass::DecodePhase));
  EXPECT_FALSE(has_tweak(capability, "transform_threshold",
                         orc::PreviewTweakClass::DecodePhase));
}
...
TEST(ChromaSinkTweakPanelTest, Preview_CapabilityListsPalDecodeTweaks) {
  ...
  EXPECT_TRUE(
      has_tweak(capability, "simple_pal", orc::PreviewTweakClass::DecodePhase));
  EXPECT_TRUE(has_tweak(capability, "transform_threshold",
                        orc::PreviewTweakClass::DecodePhase));

  EXPECT_FALSE(has_tweak(capability, "ntsc_phase_comp",
                         orc::PreviewTweakClass::DecodePhase));
  EXPECT_FALSE(has_tweak(capability, "chroma_weight",
                         orc::PreviewTweakClass::DecodePhase));
  EXPECT_FALSE(has_tweak(capability, "adapt_threshold",
                         orc::PreviewTweakClass::DecodePhase));
}
...
TEST(ChromaSinkTweakPanelTest,
  ...
  EXPECT_FALSE(has_tweak(capability, "output_path",
                         orc::PreviewTweakClass::DecodePhase));
  EXPECT_FALSE(has_tweak(capability, "output_format",
                         orc::PreviewTweakClass::DecodePhase));
  EXPECT_FALSE(has_tweak(capability, "encoder_preset",
                         orc::PreviewTweakClass::DecodePhase));
  EXPECT_FALSE(has_tweak(capability, "encoder_crf",
                         orc::PreviewTweakClass::DecodePhase));
  EXPECT_FALSE(has_tweak(capability, "encoder_bitrate",
                         orc::PreviewTweakClass::DecodePhase));
}
